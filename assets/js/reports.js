...
async function renderPerformance(force=false,resetRange=false){
 ...
 const tasks=state.tasks||[],usageStart=String(monitoringStartedAt||'2026-09-06T00:00:00Z').slice(0,10),definitionFrom=from&&from>usageStart?from:usageStart,scopedTasks=tasks.filter(t=>within(t.due_date,from,to)),definitionTasks=tasks.filter(t=>String(t?.source||'').toLowerCase()==='web'&&within(t.created_at,definitionFrom,to)),ids=[...new Set([...scopedTasks.map(t=>t.owner_id),...scopedTasks.map(t=>t.created_by),...definitionTasks.map(t=>t.created_by)])].filter(Boolean),metrics=window.bamcoDashboardMetrics;
...
function bindReportControls(){
 document.addEventListener('click',e=>{
  ...
  if(e.target.closest?.('[data-canonical-perf-clear]')){e.preventDefault();void renderPerformance(false,true);return}
...
function watchVisibility(){for(const [id,render] of [['performanceReport',renderPerformance],['responseReport',renderResponse]]){const view=q('#'+id+'View');if(!view)continue;new MutationObserver(()=>{if(state.view===id&&!view.classList.contains('hidden'))void (id==='performanceReport'?render(false,true):render(false))}).observe(view,{attributes:true,attributeFilter:['class']})}}
