...
function showNotice({title,message,version,kind='info',primary,secondary='بعداً'}){
 ...
 document.body.append(notice);notice.querySelector('.bamco-update-now')?.addEventListener('click',primary?.action);notice.querySelector('.bamco-update-later').addEventListener('click',()=>{if(version)write(keys.dismissed,version);closeNotice()});
}
...
function showAvailable(version){if(read(keys.dismissed)===version)return;showNotice({title:'نسخه جدید سامانه آماده است',message:'برای فعال‌شدن اصلاحات و امکانات جدید، سامانه را به‌روزرسانی کنید. نصب مجدد لازم نیست.',version,primary:{label:'به‌روزرسانی اکنون',action:()=>applyUpdate(version)}})}
function showSuccess(version){write(keys.installed,version);write(keys.pending,'');write(keys.attempts,'0');write(keys.dismissed,'');showNotice({title:'سامانه ارتقا یافت',message:'نسخه جدید با موفقیت فعال شد و نیازی به نصب مجدد نیست.',version,secondary:'متوجه شدم'})}
function showReinstall(version){showNotice({title:'به‌روزرسانی خودکار کامل نشد',message:'ابتدا یک‌بار دیگر تلاش کنید. اگر همچنان همین پیام را دیدید، میان‌بر سامانه را از Home Screen حذف و دوباره اضافه کنید؛ حساب و اطلاعات سامانه حذف نمی‌شود.',version,kind:'error',primary:{label:'تلاش مجدد',action:()=>applyUpdate(version)},secondary:'بعداً انجام می‌دهم'})}
async function latestVersion(){const response=await fetch(`version.json?check=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/json'}});if(!response.ok)throw Error('version check failed');const data=await response.json();return String(data?.version||'').trim()}
async function check(force=false){
 if(checking||(!force&&Date.now()-lastCheck<30000))return;checking=true;lastCheck=Date.now();
 try{const latest=await latestVersion();if(!latest||!current)return;const pending=read(keys.pending),installed=read(keys.installed),attempts=Number(read(keys.attempts)||0);if(pending&&current===pending){showSuccess(current);return}if(pending&&current!==pending&&attempts>=2){showReinstall(pending);return}if(latest!==current){showAvailable(latest);return}if(installed&&installed!==current){showSuccess(current);return}if(!installed){const returning=!!read(legacyMarker);write(keys.installed,current);if(returning)showSuccess(current)}}catch{}finally{checking=false}
}
function loadRootSyncHotfix(){
 if(document.querySelector('script[data-bamco-root-sync-hotfix]'))return;
 const script=document.createElement('script');script.dataset.bamcoRootSyncHotfix='1';script.src=`assets/js/root-sync-hotfix-20260917.js?v=${encodeURIComponent(current||'2026.09.17')}`;script.defer=true;document.body.append(script);
}
document.addEventListener('DOMContentLoaded',()=>{loadRootSyncHotfix();void check(true)},{once:true});
document.addEventListener('visibilitychange',()=>{if(!document.hidden)void check(true)});
window.addEventListener('focus',()=>void check());window.addEventListener('online',()=>void check(true));setInterval(()=>void check(),300000);
