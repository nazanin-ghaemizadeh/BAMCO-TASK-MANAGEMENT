  ...
  async function load({ ensureProfiles = true, force = false } = {}) {
    ...
    const job = (async () => {
      try {
        ...
        if (root()) root().innerHTML = `<div class="panel enterprise-error"><b>ساختار سازمانی بارگذاری نشد.</b><p>${esc(error?.message || 'خطای ناشناخته')}</p><button type="button" class="ghost content-back">بازگشت به خانه</button></div>`;
      }
    })();
