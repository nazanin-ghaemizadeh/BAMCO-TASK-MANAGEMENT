...
(()=>{
  ...
  const canFeature=(feature,action='view')=>window.BamcoAccess?.can?.(feature,action)===true;
  const denyFeature=(feature,action='view')=>window.BamcoAccess?.denied?.(feature,action);
  const isSystemManager=()=>window.BamcoAccess?.isSystemManager?.()===true;
  const requireFeature=(feature,action='view')=>{if(canFeature(feature,action))return true;denyFeature(feature,action);return false};
  const nav=q('#nav'),workspace=q('.workspace');if(!nav||!workspace)return;
  nav.insertAdjacentHTML('beforeend','<button data-view="people" data-feature-key="people"><b>♙</b><span>افراد و نقش‌ها</span></button><button data-view="templates" data-feature-key="templates"><b>≡</b><span>متن پیام‌ها</span></button><button data-view="stickers" data-feature-key="stickers"><b>◇</b><span>مدیریت استیکرها</span></button><button data-view="settings"><b>⚙</b><span>تنظیمات</span></button>');
  workspace.insertAdjacentHTML('beforeend',`
  <section id="peopleView" class="view hidden" data-feature-key="people"><div class="panel table-panel"><div class="panel-head"><div><h3>مدیریت افراد و نقش‌ها</h3><small>نقش سازمانی از جایگاه در ساختار سازمانی خوانده می‌شود؛ سطح دسترسی سامانه جداست.</small></div><input id="peopleSearch" class="search" placeholder="جست‌وجو…"></div><div class="people-actions"><button id="addPersonBtn" class="primary" data-feature-key="people" data-feature-action="create">＋ افزودن فرد</button><button id="editPersonBtn" class="ghost" data-feature-key="people" data-feature-action="edit">ویرایش</button><button id="deletePersonBtn" class="danger" data-feature-key="people" data-feature-action="delete">حذف</button></div><div class="table-wrap"><table class="manager-table"><thead><tr><th>فرد</th><th>عکس پروفایل</th><th>نقش سازمانی</th><th>سمت سازمانی</th><th>دسترسی سامانه</th><th>جنسیت</th><th>شماره همراه</th><th>شماره داخلی</th><th>پست الکترونیک سازمانی</th><th>نام کاربری</th><th>رمز عبور</th><th>عنوان خطاب</th><th>فعال</th></tr></thead><tbody id="peopleBody"></tbody></table></div></div></section>
  <section id="templatesView" class="view hidden" data-feature-key="templates"><div class="panel"><div class="panel-head"><div><h3>متن پیام‌ها</h3><small>متن پیام داخل سامانه بر اساس وضعیت متولی</small></div><div class="manager-toolbar"><select id="templateState" data-feature-key="templates" data-feature-action="view"><option value="state1">وضعیت مطلوب</option><option value="state2">یادآوری</option><option value="state3">نیازمند توجه</option><option value="state4">پیگیری جدی</option><option value="state5">اقدام فوری</option><option value="followup">یادآوری مجدد</option></select><button id="saveTemplateBtn" class="primary" data-feature-key="templates" data-feature-action="edit">ذخیره متن</button></div></div><div class="manager-note">متغیرها: [عنوان مخاطب]، [تعداد هشدار]، [تعداد دیرکرد]، [جدول امور هشداری]، [جدول امور دیرکردی]، [استیکر] و [تاریخ گزارش]</div><textarea id="templateBody" class="template-editor" data-feature-key="templates" data-feature-action="edit"></textarea></div></section>
  <section id="stickersView" class="view hidden" data-feature-key="stickers"><div class="panel"><div class="panel-head"><div><h3>مدیریت استیکرها</h3><small>نسخه را تعریف کنید و ده تصویر آن را در پایگاه داده بارگذاری کنید.</small></div></div><div id="stickerGrid" hidden></div></div></section>
  <section id="settingsView" class="view hidden"><div class="panel"><div class="panel-head"><div><h3>تنظیمات حساب کاربری</h3><small>نام نمایشی، پست الکترونیک سازمانی، تصویر پروفایل و امنیت حساب</small></div></div><div class="manager-form"><label>نام نمایشی<input id="profileDisplayName"></label><label>پست الکترونیک سازمانی<input id="profileEmail" class="english" dir="ltr" type="email" autocomplete="email"></label><label>نام کاربری ورود<input id="profileLoginName" class="english" dir="ltr" autocomplete="username" spellcheck="false"></label><div class="profile-photo-field"><div id="profileAvatarPreview" class="profile-avatar-preview">ب</div><div><b>تصویر پروفایل</b><p>PNG، JPG یا WebP تا ۸ مگابایت</p><button id="chooseAvatarBtn" type="button" class="ghost">انتخاب و تنظیم عکس</button><input id="profileAvatar" type="file" accept="image/png,image/jpeg,image/webp" hidden></div></div><div class="account-security-box"><div><h4>تغییر رمز عبور</h4><p>رمز حساب خود را از این بخش تغییر دهید.</p></div><button id="changePasswordBtn" type="button" class="primary">تغییر رمز عبور</button></div></div><div class="modal-actions"><button id="saveProfileBtn" class="primary">ذخیره تنظیمات حساب</button></div></div></section>
  <dialog id="personDialog" class="modal manager-modal"><form id="personForm"><div class="modal-head"><div><h3 id="personDialogTitle">افزودن فرد</h3><p>نقش سازمانی پس از ثبت شخص، از طریق جایگاه او در ساختار سازمانی تعیین می‌شود.</p></div><button type="button" data-person-close>×</button></div><div class="manager-form"><label>نام فرد<input name="full_name" required></label><label>سطح دسترسی سامانه<select name="system_access"><option value="owner">کاربر سامانه</option><option value="manager">مدیر سامانه</option></select></label><label>جنسیت<select name="gender"><option>آقا</option><option>خانم</option></select></label><label>شماره همراه<input name="mobile_phone" class="contact-number" inputmode="tel" dir="ltr"></label><label>شماره داخلی<input name="internal_extension" class="contact-number" inputmode="numeric" dir="ltr"></label><label>پست الکترونیک سازمانی (اختیاری)<input name="email" class="english" dir="ltr" type="email" placeholder="name@bamco.ir"></label><label>عنوان خطاب<input name="salutation"></label><label class="person-message-channel">کانال پیش‌فرض پیام<select name="default_message_channel"><option value="portal">داخل سامانه</option><option value="email">ایمیل</option><option value="both">هر دو</option></select><small>بدون ایمیل، پیام فقط داخل سامانه ارسال می‌شود.</small></label><label>فعال<select name="active"><option value="true">بله</option><option value="false">خیر</option></select></label></div><div id="personLoginActions" class="modal-actions hidden"><button id="editPersonLoginBtn" class="ghost" type="button">نام کاربری و رمز موقت</button></div><p id="personError" class="form-error" role="alert"></p><div class="modal-actions"><button type="button" class="ghost" data-person-close>انصراف</button><button type="submit" class="primary">ذخیره</button></div></form></dialog>`);
  document.body.append(q('#personDialog'));
  q('#personForm').addEventListener('input',event=>{
    const input=event.target;if(!input.matches?.('input.contact-number'))return;
    const cursor=input.selectionStart;input.value=faDigits(input.value);
    if(cursor!==null)input.setSelectionRange(cursor,cursor);
  });
  q('#settingsView .manager-form').insertAdjacentHTML('afterend',`<section id="homeLayoutSettings" class="home-layout-settings" aria-labelledby="homeLayoutTitle">
    <div class="home-layout-settings-head"><h4 id="homeLayoutTitle">چیدمان صفحه اصلی پس از ورود</h4></div>
    <div class="home-layout-choices" role="radiogroup" aria-label="نوع چیدمان صفحه اصلی">
      <label><input type="radio" name="homeLayoutMode" value="launcher"><span><b>آیکون‌های دایره‌ای</b></span></label>
      <label><input type="radio" name="homeLayoutMode" value="cards"><span><b>کارت‌های باز</b></span></label>
      <label><input type="radio" name="homeLayoutMode" value="custom"><span><b>چیدمان دلخواه</b></span></label>
    </div>
    <div class="home-layout-order-panel" hidden><ol id="homeLayoutOrder" aria-label="ترتیب کارت‌ها"></ol></div>
  </section>`);
  ...
  function syncPersonSelection(){
    ...
    const canEdit=canFeature('people','edit'),canDelete=canFeature('people','delete')&&isSystemManager();
    q('#editPersonBtn').disabled=!canEdit||!people.length;q('#editPersonBtn').title=selected.size>1?'ویرایش فقط برای یک فرد مجاز است':'ویرایش فرد';q('#deletePersonBtn').disabled=!canDelete||!selected.size;
  }
  ...
  async function edge(body={},method='POST'){const result=await api('/functions/v1/admin-users',method==='GET'?{method}:{method,body});if(!result?.ok)throw new Error(result?.error||'عملیات حساب کاربری تأیید نشد.');return result}
  ...
  function syncProfiles(rows,{replaceAll=false}={}){
    // BamcoProfiles is the canonical identity store. The table view keeps only
    // references to its rows; it never owns a second profile copy.
    if(window.BamcoProfiles){
      const merged=window.BamcoProfiles.upsert(rows,{replaceAll,source:'people-directory'}),known=new Map(people.map(person=>[String(person.id),person]));
      for(const person of merged)known.set(String(person.id),person);
      people=replaceAll?merged:[...known.values()];
      void window.bamcoMedia?.avatars(document,people);
      return window.BamcoProfiles.list();
    }
  ...
  async function loadPeople({refreshOrganization=true}={}){
    if(!canFeature('people','view'))return [];
    if(peopleLoad)return peopleLoad;
    const user=state.user?.id,session=window.bamcoAuth?.snapshot?.();
    const job=(async()=>{
      const directory=await edge({},'GET'),rows=Array.isArray(directory?.profiles)?directory.profiles:[];
  ...
  async function refreshProfileMetadata(force=false){
    ...
    if(window.BamcoProfiles?.refreshCurrent){
      if(canFeature('people','view')&&!q('#peopleView').classList.contains('hidden'))return loadPeople();
      return window.BamcoProfiles.refreshCurrent({force,reason:'shell'});
    }
  ...
  function credentialDate(person){return person.password_changed_at||person.updated_at||null}
  function credentialExportState(person){const state=credentialState(person),date=credentialDate(person);return date?`${state} — آخرین به‌روزرسانی: ${new Date(date).toLocaleString('fa-IR')}`:state}
  // In the People & Roles directory, the organizational email is the visible
  // username whenever it exists. Accounts without an email retain their safe
  // generated login identifier, which the system manager can still manage.
  function faDigits(value){return String(value).replace(/[0-9٠-٩]/g,d=>/\d/.test(d)?'۰۱۲۳۴۵۶۷۸۹'[d]:'۰۱۲۳۴۵۶۷۸۹'['٠١٢٣٤٥٦٧٨٩'.indexOf(d)])}
  function latinDigits(value){return String(value).replace(/[۰-۹٠-٩]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)>=0?'۰۱۲۳۴۵۶۷۸۹'.indexOf(d):'٠١٢٣٤٥٦٧٨٩'.indexOf(d)))}
  function peopleUsername(person){return person.login_name||person.email||'—'}
  function peopleExportRows(){const showCredentials=isSystemManager();return people.map(person=>({id:String(person.id),values:[person.full_name||'',profilePhotoState(person),organizationRole(person),organizationPosition(person),systemAccess(person),person.gender||'—',faDigits(person.mobile_phone||'—'),faDigits(person.internal_extension||'—'),person.email||'—',peopleUsername(person),showCredentials?credentialExportState(person):'—',person.salutation||'—',person.active!==false?'بله':'خیر']}))}
  window.bamcoTableData=window.bamcoTableData||{};window.bamcoTableData.peopleView=peopleExportRows;
  function renderPeople(){const term=q('#peopleSearch').value.trim().toLowerCase(),rows=people.filter(p=>!term||[p.full_name,p.email,p.login_name,p.salutation,p.mobile_phone,p.internal_extension,organizationRole(p),organizationPosition(p)].some(v=>String(v||'').toLowerCase().includes(term))),showCredentials=isSystemManager();q('#peopleBody').innerHTML=rows.map(p=>{const initial=esc(String(p.display_name||p.full_name||'ب').trim().charAt(0)||'ب'),credential=credentialState(p),credentialCell=showCredentials?`<button type="button" class="ghost" data-person-credentials="${esc(p.id)}" title="ویرایش نام کاربری و تعیین رمز موقت">${credential}</button>${credentialDate(p)?`<small class="credential-date">آخرین به‌روزرسانی: ${esc(new Date(credentialDate(p)).toLocaleString('fa-IR'))}</small>`:''}`:'—';return `<tr data-id="${p.id}" class="${selected.has(p.id)?'person-selected':''}"><td>${esc(p.full_name)}</td><td class="people-avatar-cell"><span class="people-avatar" data-profile-photo="${esc(p.id)}" aria-label="تصویر ${esc(p.full_name)}" title="${esc(profilePhotoState(p))}">${initial}</span></td><td>${esc(organizationRole(p))}</td><td>${esc(organizationPosition(p))}</td><td>${esc(systemAccess(p))}</td><td>${esc(p.gender||'—')}</td><td class="people-contact" dir="ltr">${esc(faDigits(p.mobile_phone||'—'))}</td><td class="people-contact" dir="ltr">${esc(faDigits(p.internal_extension||'—'))}</td><td class="english">${esc(p.email||'—')}</td><td class="english" dir="ltr">${esc(peopleUsername(p))}</td><td>${credentialCell}</td><td>${esc(p.salutation||'—')}</td><td>${p.active!==false?'بله':'خیر'}</td></tr>`}).join('')||'<tr><td colspan="13" class="empty">کاربری ثبت نشده است.</td></tr>';syncPersonSelection();if(window.bamcoMedia?.avatars)void window.bamcoMedia.avatars(q('#peopleBody'),rows)}
  function syncMessageAvailability(){const f=q('#personForm'),hasEmail=!!f.elements.email.value.trim(),channel=f.elements.default_message_channel;channel.disabled=false;[...channel.options].forEach(o=>o.disabled=o.value!=='portal'&&!hasEmail);if(!hasEmail||!channel.value)channel.value='portal';q('#personLoginActions').classList.toggle('hidden',!editing||!isSystemManager())}
  function syncPersonPrivilegeFields(){const f=q('#personForm'),system=isSystemManager(),access=f.elements.system_access,active=f.elements.active;if(access){access.disabled=!system;access.setAttribute('aria-disabled',String(!system))}if(active){active.disabled=!system;active.setAttribute('aria-disabled',String(!system))}q('#personLoginActions').classList.toggle('hidden',!editing||!system)}
  function openPerson(p=null){const action=p?'edit':'create';if(!requireFeature('people',action))return;if(!p&&!isSystemManager())return toast('تعریف کاربر جدید فقط برای مدیر سامانه مجاز است.',true);editing=p;const f=q('#personForm');f.reset();q('#personError').textContent='';f.elements.system_access.value='owner';f.elements.active.value='true';if(p){for(const k of ['full_name','gender','mobile_phone','internal_extension','email','salutation','active','default_message_channel'])if(f.elements[k])f.elements[k].value=['mobile_phone','internal_extension'].includes(k)?faDigits(p[k]??''):String(p[k]??'');f.elements.system_access.value=p.role==='manager'?'manager':'owner'}q('#personDialogTitle').textContent=p?'ویرایش فرد':'افزودن فرد';syncPersonPrivilegeFields();syncMessageAvailability();q('#personDialog').showModal()}
  function setFeatureControl(node,allowed){if(!node)return;node.classList.toggle('hidden',!allowed);if('disabled' in node)node.disabled=!allowed;node.setAttribute('aria-hidden',allowed?'false':'true');node.setAttribute('aria-disabled',allowed?'false':'true')}
  function stickerAction(target){
    const node=target?.nodeType===1?target:target?.parentElement;
    if(!node)return null;
    if(node.matches?.('#stickerPackForm')||node.closest?.('#stickerSave,.sticker-pick,#stickerFile'))return q('#stickerPackDialog')?.dataset.stickerOperation||'edit';
    if(node.closest?.('#stickerNew'))return'create';
    if(node.closest?.('#stickerEdit,#stickerActivate'))return'edit';
    return null;
  }
  function syncStickerControls(){
    const view=canFeature('stickers','view'),operation=q('#stickerPackDialog')?.dataset.stickerOperation||'edit';
    for(const node of document.querySelectorAll('#stickerSet,#stickerState'))setFeatureControl(node,view);
    for(const node of document.querySelectorAll('#stickerNew')){node.dataset.featureKey='stickers';node.dataset.featureAction='create';setFeatureControl(node,view&&canFeature('stickers','create'))}
    for(const node of document.querySelectorAll('#stickerEdit,#stickerActivate')){node.dataset.featureKey='stickers';node.dataset.featureAction='edit';setFeatureControl(node,view&&canFeature('stickers','edit'))}
    for(const node of document.querySelectorAll('#stickerSave,.sticker-pick,#stickerFile')){node.dataset.featureKey='stickers';node.dataset.featureAction=operation;setFeatureControl(node,view&&canFeature('stickers',operation))}
  }
  function syncFeatureControls(){
    const canPeopleView=canFeature('people','view'),canPeopleEdit=canFeature('people','edit'),canPeopleCreate=canFeature('people','create')&&isSystemManager(),canPeopleDelete=canFeature('people','delete')&&isSystemManager();
    setFeatureControl(q('#addPersonBtn'),canPeopleCreate);setFeatureControl(q('#editPersonBtn'),canPeopleEdit);setFeatureControl(q('#deletePersonBtn'),canPeopleDelete);
    for(const node of document.querySelectorAll('#peopleView [data-management-export]')){node.dataset.featureKey='people';node.dataset.featureAction='export';setFeatureControl(node,canFeature('people','export'))}
    syncPersonPrivilegeFields();syncPersonSelection();syncStickerControls();
    // A grant can change while a dialog is open.  Calling the dialog method
    // itself must be optional: older embedded webviews (and test DOMs) can
    // expose the element before implementing HTMLDialogElement#close.
    if(!canPeopleView){selected.clear();q('#personDialog')?.close?.();q('#personPickerDialog')?.close?.()}
    if(!canFeature('templates','view'))q('#templatesView')?.classList.add('hidden');
    if(!canFeature('stickers','view')){q('#stickersView')?.classList.add('hidden');q('#stickerPackDialog')?.close?.()}
  }
  function guardFeatureActions(event){
    const exportButton=event.target?.closest?.('#peopleView [data-management-export]');
    if(exportButton&&!canFeature('people','export')){event.preventDefault();event.stopImmediatePropagation();denyFeature('people','export');return}
    const action=stickerAction(event.target);if(!action)return;
    if(event.type==='click'){
      const stickerDialog=q('#stickerPackDialog');
      if(event.target.closest?.('#stickerNew')&&stickerDialog)stickerDialog.dataset.stickerOperation='create';
      if(event.target.closest?.('#stickerEdit')&&stickerDialog)stickerDialog.dataset.stickerOperation='edit';
    }
    if(!canFeature('stickers','view')||!canFeature('stickers',action)){event.preventDefault();event.stopImmediatePropagation();denyFeature('stickers',action);return}
    queueMicrotask(syncStickerControls);
  }
  function pickPersonToEdit(){
    if(!requireFeature('people','edit'))return;
  ...
  async function savePerson(e){
    e.preventDefault();const form=e.currentTarget,submit=form.querySelector('[type="submit"]'),action=editing?'edit':'create';if(submit.disabled)return;if(!requireFeature('people',action))return;if(!editing&&!isSystemManager())return toast('تعریف کاربر جدید فقط برای مدیر سامانه مجاز است.',true);
    const data=Object.fromEntries(new FormData(form)),system=isSystemManager();if(system&&data.system_access)data.role=data.system_access;delete data.system_access;if('active' in data)data.active=data.active==='true';data.mobile_phone=latinDigits(data.mobile_phone||'').replace(/\s+/g,'').trim()||null;data.internal_extension=latinDigits(data.internal_extension||'').replace(/\s+/g,'').trim()||null;data.email=String(data.email||'').trim()||null;data.messaging_enabled=true;data.default_message_channel=data.email?(data.default_message_channel||'portal'):'portal';if(editing)data.user_id=editing.id;
  ...
  function showInitialCredentials(result){
    if(!isSystemManager())return toast('مدیریت نام کاربری و رمز عبور فقط برای مدیر سامانه مجاز است.',true);
    ...
    form.onsubmit=async event=>{
      ...
      try{const saved=await edge({action:'save_credentials',user_id:result.id,login_name:form.elements.login_name.value,temporary_password:form.elements.temporary_password.value});form.elements.login_name.value=saved.login_name;q('#initialCredentialsTitle').textContent='اطلاعات ورود ذخیره شد';result.login_name=saved.login_name;window.BamcoProfiles?.upsert?.([{...(window.BamcoProfiles.get(result.id)||result),login_name:saved.login_name}],{source:'manager-credential-save',broadcast:true});await loadPeople();if(state.profile.id===result.id){if(!window.BamcoProfiles)state.profile.login_name=saved.login_name;state.user.email=saved.login_name.includes('@')?saved.login_name:saved.login_name+'@no-email.invalid'}void window.bamcoInbox?.load();}
  ...
  async function editPersonLogin(){
    if(!editing||!isSystemManager())return;const personId=editing.id,button=q('#editPersonLoginBtn');button.disabled=true;
  ...
  async function removePerson(){
    if(deletingPeople)return;if(!requireFeature('people','delete'))return;if(!isSystemManager())return toast('حذف حساب فقط برای مدیر سامانه مجاز است.',true);const chosen=people.filter(p=>selected.has(p.id));if(!chosen.length)return toast('حداقل یک فرد را انتخاب کنید.',true);
    ...
    try{
      ...
      if(window.BamcoProfiles)window.BamcoProfiles.remove([...deletedIds],{source:'people-delete'});
      people=people.filter(p=>!deletedIds.has(p.id));state.profiles=(state.profiles||[]).filter(p=>!deletedIds.has(p.id));renderPeople();window.bamcoSelection.set('#peopleBody',failed);
  ...
  async function saveProfile(){
    ...
    const profile=window.BamcoProfiles?.current?.()||state.profile,user=state.user?.id,profileId=profile?.id,session=window.bamcoAuth?.snapshot?.(),savedBlob=pendingBlob;
    ...
    try{
      ...
      if(login!==(profile.login_name||state.user?.email||'')){
        const saved=await edge({action:'save_own_login',login_name:login});
        if(!current())return;
        window.BamcoProfiles?.upsert?.([{...profile,login_name:saved.login_name}],{source:'login-name-save',broadcast:true});
        if(!window.BamcoProfiles)state.profile.login_name=saved.login_name;
        state.user.email=saved.login_name.includes('@')?saved.login_name:saved.login_name+'@no-email.invalid';
        void window.bamcoInbox?.load();
      }
      const display_name=q('#profileDisplayName').value.trim(),email=q('#profileEmail').value.trim();
      if(email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))throw Error('پست الکترونیک سازمانی معتبر نیست.');
      let avatar_path=(window.BamcoProfiles?.current?.()||state.profile).avatar_path||null;
      if(savedBlob){
      ...
      let savedResult;
      try{
        savedResult=await rpc('update_my_profile',{p_display_name:display_name,p_organizational_email:email||null,p_avatar_path:avatar_path});
      }catch(error){
        // Older deployed databases may not yet have the canonical RPC. Keep the
        // previous safe behaviour only when the organisational email is unchanged.
        const missing=/function|schema cache|not found|404/i.test(String(error?.message||''));
        if(!missing||email!==String(profile.email||''))throw error;
        savedResult=await update('profiles',`id=eq.${profileId}`,{display_name,avatar_path,updated_at:new Date().toISOString()});
      }
      if(!current())return;
      const saved=Array.isArray(savedResult)?savedResult.find(p=>p.id===profileId)||savedResult[0]:(savedResult?.profile||savedResult);
      ...
      syncProfiles([{...(window.BamcoProfiles?.current?.()||state.profile),...saved}]);
      if(!q('#peopleView').classList.contains('hidden'))renderPeople();
      q('#userName').textContent=(window.BamcoProfiles?.current?.()||state.profile).display_name||(window.BamcoProfiles?.current?.()||state.profile).full_name;
  ...
  function loadSettings(){const profile=window.BamcoProfiles?.current?.()||state.profile;q('#profileDisplayName').value=profile.display_name||profile.full_name||'';q('#profileEmail').value=profile.email||'';q('#profileLoginName').value=profile.login_name||state.user?.email||'';window.refreshProfileAvatar()}
  window.bamcoPeople={refresh:loadPeople,syncProfiles,refreshProfileMetadata,async open(id){if(!requireFeature('people','view'))return;showView('people');await loadPeople();const person=people.find(p=>p.id===id);if(person){window.bamcoSelection.set('#peopleBody',[id]);if(canFeature('people','edit'))openPerson(person)}}};
  window.bamcoAccount={open(){showView('settings');loadSettings()}};
  document.addEventListener('bamco:profiles-updated',event=>{
    const store=window.BamcoProfiles;if(!store)return;
    people=people.map(person=>store.get(person.id)).filter(Boolean);
    const current=store.current();
    if(current&&q('#userName'))q('#userName').textContent=current.display_name||current.full_name||current.email||'';
    if(canFeature('people','view')&&!q('#peopleView').classList.contains('hidden'))renderPeople();
  });
  window.addEventListener('bamco:feature-access-changed',syncFeatureControls);
  document.addEventListener('click',guardFeatureActions,true);
  document.addEventListener('submit',guardFeatureActions,true);
  document.addEventListener('change',guardFeatureActions,true);
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    q('#nav').addEventListener('click',e=>{const view=e.target.closest('button[data-view]')?.dataset.view;if(view==='people'&&canFeature('people','view'))loadPeople().catch(err=>toast(err.message,true));if(view==='settings')setTimeout(loadSettings)});
    document.addEventListener('bamco:organization-updated',()=>{if(canFeature('people','view')&&!q('#peopleView').classList.contains('hidden'))renderPeople()});
    queueMicrotask(syncFeatureControls);
  });
})();
...
(()=>{
 ...
 nav.insertAdjacentHTML('beforeend','<button data-view="messages"><b>✉</b><span>پیام‌ها</span><i id="messageBadge" class="message-badge"></i></button><button data-view="sentMessages" data-feature-key="sentMessages"><b>✓</b><span>پیام‌های ارسال‌شده</span></button>');
 workspace.insertAdjacentHTML('beforeend','<section id="messagesView" class="view hidden"><div class="panel"><div class="panel-head"><h3>پیام‌های من</h3></div><div class="manager-toolbar"><button id="refreshMessages" class="ghost">تازه‌سازی</button><button id="clearAllMessages" class="danger" type="button" disabled>پاک کردن همه اعلان‌ها</button></div><div id="messageList" class="message-list"></div></div></section><section id="sentMessagesView" class="view hidden" data-feature-key="sentMessages"></section>');
 let loading=null;
 const notificationText=(_notice,value)=>fa(value??'');
 ...
 q('#messageList').innerHTML=alerts.map(n=>{const title=kinds[n.notification_type]&&n.entity_type==='chat_thread'&&!['daily','reminder','manual','system'].includes(n.notification_type)?threadNames.get(n.entity_id)||n.title:n.title;return `<article tabindex="0" role="button" class="message-card message-notification ${n.read_at?'':'unread'}" data-notification="${n.id}"><div class="message-meta"><span class="message-kind">${esc(kinds[n.notification_type]||'اعلان سامانه')}</span><time>${jalaliDateTime(n.created_at)}</time><button type="button" class="message-dismiss" data-dismiss-notification="${n.id}" aria-label="حذف اعلان از پیام‌های من" title="حذف اعلان">×</button></div><h4>${esc(notificationText(n,title))}</h4><p>${esc(notificationText(n,n.body||''))}</p><span class="message-open">باز کردن ${n.entity_type==='chat_thread'?'گفت‌وگو':'جزئیات'} ←</span></article>`}).join('')+rows.map(x=>{const m=x.portal_messages||{},s=byMessage.get(String(x.message_id));return `<article class="message-card ${x.read_at?'':'unread'}" data-mid="${x.message_id}"><div class="message-meta"><b>${esc(fa(m.subject||''))}</b><span>${jalaliDateTime(m.created_at)}</span><button type="button" class="message-dismiss" data-dismiss-message="${x.message_id}" aria-label="حذف پیام از پیام‌های من" title="حذف پیام">×</button></div><div class="message-report-body">${s?BamcoMessageRender.html(s):`<p style="white-space:pre-wrap">${esc(fa(m.body||''))}</p>`}</div><div class="message-actions">${m.require_ack&&!x.acknowledged_at?'<button class="ghost ack-message">تأیید دریافت</button>':''}${m.allow_reply?`<button class="ghost reply-message">${x.replied_at?'ویرایش پاسخ':'پاسخ'}</button><div class="message-reply-box hidden"><textarea aria-label="پاسخ شما" placeholder="پاسخ شما…">${esc(x.reply_text||'')}</textarea><button class="primary save-reply">ثبت پاسخ</button></div>`:''}${x.replied_at?'<small>پاسخ شما ثبت شده است.</small>':''}</div></article>`}).join('')||'<div class="empty">پیامی برای شما ثبت نشده است.</div>';
