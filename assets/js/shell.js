...
(()=>{
  ...
  q('#settingsView .manager-form').insertAdjacentHTML('afterend',`<section id="homeLayoutSettings" class="home-layout-settings" aria-labelledby="homeLayoutTitle">
    <div class="home-layout-settings-head"><h4 id="homeLayoutTitle">چیدمان صفحهٔ اصلی پس از ورود</h4><p>نمای کارت‌ها و شیوهٔ باز شدن تب‌های هر کارت را انتخاب کنید. تغییر بلافاصله اعمال می‌شود.</p></div>
    <div class="home-layout-choices" role="radiogroup" aria-label="نوع چیدمان صفحهٔ اصلی">
      <label><input type="radio" name="homeLayoutMode" value="launcher"><span><b>آیکون‌های دایره‌ای</b><small>تب‌های هر کارت در یک پنجره باز می‌شوند.</small></span></label>
      <label><input type="radio" name="homeLayoutMode" value="cards"><span><b>کارت‌های باز</b><small>همهٔ تب‌های مجاز زیر عنوان کارت دیده می‌شوند.</small></span></label>
      <label><input type="radio" name="homeLayoutMode" value="custom"><span><b>چیدمان دلخواه</b><small>آیکون‌ها را به ترتیب دلخواه خود بچینید.</small></span></label>
    </div>
    <div class="home-layout-order-panel" hidden><p>با کشیدن آیکون‌ها در صفحهٔ اصلی یا دکمه‌های زیر ترتیب را تغییر دهید.</p><ol id="homeLayoutOrder" aria-label="ترتیب کارت‌ها"></ol></div>
    <small class="home-layout-storage-note">چیدمان برای حساب شما در همین مرورگر ذخیره می‌شود.</small>
  </section>`);
  ...
  async function savePerson(e){
    ...
    const data=Object.fromEntries(new FormData(form)),system=isSystemManager();if(system&&data.system_access)data.role=data.system_access;delete data.system_access;if('active' in data)data.active=data.active==='true';data.mobile_phone=String(data.mobile_phone||'').replace(/\s+/g,'').trim()||null;data.internal_extension=String(data.internal_extension||'').replace(/\s+/g,'').trim()||null;data.email=String(data.email||'').trim()||null;data.messaging_enabled=true;data.default_message_channel=data.email?(data.default_message_channel||'portal'):'portal';if(editing)data.user_id=editing.id;
