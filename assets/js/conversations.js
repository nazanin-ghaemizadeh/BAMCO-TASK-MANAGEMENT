...
(()=>{
 ...
 function openTaskDate(host,which){
  const selector=which==='from'?'from':'to',visible=q(`[data-task-${selector}]`,host),hidden=q(`[data-task-${selector}-iso]`,host);
  window.bamcoEnterprise.openJalaliPicker({visible,hidden,label:which==='from'?'از تاریخ':'تا تاریخ'});
 }
 ...
 async function newConversation(view){const feature=view.id.replace(/View$/,'');if(!can(feature,'create'))return denied(feature,'create');selectedTask=null;window.bamcoChat.close();const host=q('.conversation-stage',view);loading(host);const people=(await directory()).filter(p=>p.id!==state.user.id);if(!host.isConnected)return;
  if(view.id==='taskChatsView'){
   ...
   host.innerHTML=`<div class="conversation-recipient-head"><h4>انتخاب وظیفه</h4><div class="conversation-task-filters"><label>منبع<select data-task-source><option value="kanban">کانبان</option><option value="archive">آرشیو</option></select></label><label>متولی<select data-task-owner><option value="">همه متولی‌ها</option>${owners.map(p=>`<option value="${esc(p.id)}">${esc(label(p))}</option>`).join('')}</select></label><label>از تاریخ<span class="conversation-date-field"><input type="text" class="jalali-input" readonly data-task-from placeholder="انتخاب تاریخ شمسی"><input type="hidden" data-task-from-iso><button type="button" class="calendar-btn" data-task-calendar="from" aria-label="تقویم از تاریخ">▦</button></span></label><label>تا تاریخ<span class="conversation-date-field"><input type="text" class="jalali-input" readonly data-task-to placeholder="انتخاب تاریخ شمسی"><input type="hidden" data-task-to-iso><button type="button" class="calendar-btn" data-task-calendar="to" aria-label="تقویم تا تاریخ">▦</button></span></label><button type="button" class="ghost" data-task-date-clear>حذف بازه</button></div></div><div class="conversation-task-choices"></div>`;
   const palette=taskPalette(tasks.length),taskColors=new Map(tasks.map((t,index)=>[t.id,palette[index]]));
   const paint=()=>{const owner=q('[data-task-owner]',host).value,source=q('[data-task-source]',host).value,from=q('[data-task-from-iso]',host).value,to=q('[data-task-to-iso]',host).value;q('.conversation-task-choices',host).innerHTML=tasks.filter(t=>!!t.archived===(source==='archive')&&(!owner||String(t.owner_id||t.created_by)===owner)&&(!from||String(t.start_date||t.created_at||'').slice(0,10)>=from)&&(!to||String(t.start_date||t.created_at||'').slice(0,10)<=to)).map(t=>`<button type="button" class="conversation-task-tile" style="--task-fill:${taskColors.get(t.id).fill};--task-ink:${taskColors.get(t.id).ink}" data-task-choice="${t.id}" aria-label="${esc(t.title)}" data-preview="${esc([t.title,t.description,'متولی: '+ownerName(t),t.status].filter(Boolean).join('\n'))}"><span class="conversation-task-id">${esc(digits(t.legacy_id||t.id))}</span></button>`).join('')||'<div class="conversation-empty">وظیفه‌ای در این بازه وجود ندارد.</div>'};paint();host.querySelectorAll('[data-task-owner],[data-task-source]').forEach(input=>input.addEventListener('change',paint));
   host.querySelectorAll('[data-task-from],[data-task-to]').forEach(input=>input.addEventListener('input',paint));
   host.querySelectorAll('[data-task-calendar], [data-task-from], [data-task-to]').forEach(button=>button.addEventListener('click',()=>openTaskDate(host,button.dataset.taskCalendar|| (button.hasAttribute('data-task-from')?'from':'to'))));
   q('[data-task-date-clear]',host).onclick=()=>{for(const input of qa('[data-task-from],[data-task-to],[data-task-from-iso],[data-task-to-iso]',host))input.value='';paint()};
