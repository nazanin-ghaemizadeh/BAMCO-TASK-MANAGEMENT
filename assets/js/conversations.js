...
(()=>{
 ...
 function openTaskDate(host,which,refresh){
  let dialog=q('#conversationJalaliDialog');if(!dialog){dialog=document.createElement('dialog');dialog.id='conversationJalaliDialog';dialog.className='modal small bamco-dialog';dialog.innerHTML='<form method="dialog"><div class="modal-head"><h3>انتخاب تاریخ شمسی</h3></div><div class="conversation-jalali-selects"><label>سال<select name="year"></select></label><label>ماه<select name="month"></select></label><label>روز<select name="day"></select></label></div><div class="modal-actions"><button type="button" data-jalali-clear>پاک کردن</button><button type="button" data-jalali-close>انصراف</button><button type="submit" class="primary">تأیید</button></div></form>';document.body.append(dialog)}
  const input=q(which==='from'?'[data-task-from]':'[data-task-to]',host),form=q('form',dialog),now=typeof currentJalali==='function'?currentJalali():{y:1405,m:1,d:1},p=input.dataset.iso&&typeof persianParts==='function'?persianParts(input.dataset.iso):now;
  form.elements.year.innerHTML=Array.from({length:16},(_,i)=>now.y-5+i).map(y=>`<option value="${y}">${digits(y)}</option>`).join('');form.elements.month.innerHTML=['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'].map((m,i)=>`<option value="${i+1}">${m}</option>`).join('');
  form.elements.year.value=String(p.y);form.elements.month.value=String(p.m);
  const fillDays=()=>{const max=typeof daysInJalaliMonth==='function'?daysInJalaliMonth(form.elements.year.value,form.elements.month.value):31,day=Math.min(Number(form.elements.day.value)||p.d,max);form.elements.day.innerHTML=Array.from({length:max},(_,i)=>`<option value="${i+1}">${digits(i+1)}</option>`).join('');form.elements.day.value=String(day)};fillDays();form.elements.year.onchange=fillDays;form.elements.month.onchange=fillDays;
  form.onsubmit=event=>{event.preventDefault();const iso=jalaliToISO(form.elements.year.value,form.elements.month.value,form.elements.day.value);if(!iso)return;input.dataset.iso=iso;input.value=jalaliText(iso);dialog.close();refresh()};q('[data-jalali-close]',dialog).onclick=()=>dialog.close();q('[data-jalali-clear]',dialog).onclick=()=>{input.value='';delete input.dataset.iso;dialog.close();refresh()};dialog.showModal();
 }
 ...
 function threadButton(t){const kind=t.system_recipient_id?'system':t.thread_type,task=t.task_id?(state.tasks||[]).find(x=>String(x.id)===String(t.task_id)):null,sub=t.system_recipient_id?'پیام‌های خودکار · قابل پاسخ':t.task_id?'وظیفه '+digits(task?.legacy_id||t.task_id)+' · '+(task?.title||''):kind==='public'?'عمومی · همه کاربران':kind==='group'?'گروه':'خصوصی',preview=t.last_message?.startsWith('BAMCO_')?'پیوست یا استیکر':digits(t.last_message||'');return `<button type="button" class="conversation-item ${t.task_id?'conversation-task':''}" data-thread="${esc(t.id)}" data-title="${esc(t.title)}" data-kind="${kind}" data-task-id="${t.task_id||''}" data-person-id="${esc(t.person_id||'')}" data-group-photo="${esc(t.avatar_path||'')}" data-read-only="${t.is_active===false}" data-last-read-at="${esc(t.last_read_at||'')}" data-unread-count="${Number(t.unread_count)||0}"><span class="conversation-avatar" ${kind==='public'||kind==='system'?'':kind==='group'?`data-thread-photo="${esc(t.id)}"`:t.person_id?`data-profile-photo="${esc(t.person_id)}"`:''}>${kind==='system'?'◉':kind==='public'?'<img src="assets/images/bamco-icon-192.png" alt="لوگوی شرکت">':kind==='group'?'♙':esc(t.title?.[0]||'گ')}</span><span><strong>${esc(t.title)}</strong><small>${esc(sub)}</small>${preview?`<small class="conversation-preview">${esc(preview)}</small>`:''}</span>${Number(t.unread_count)?`<b class="conversation-unread">${digits(t.unread_count)}</b>`:''}</button>`}
 ...
 async function newConversation(view){const feature=view.id.replace(/View$/,'');if(!can(feature,'create'))return denied(feature,'create');selectedTask=null;window.bamcoChat.close();const host=q('.conversation-stage',view);loading(host);const people=(await directory()).filter(p=>p.id!==state.user.id);if(!host.isConnected)return;
  if(view.id==='taskChatsView'){
   ...
   const tasks=[...(state.tasks||[])].sort(compareTaskNumbers),ownerIds=new Set(tasks.map(task=>String(task.owner_id||task.created_by||'')).filter(Boolean));
   const owners=[...new Map([...(state.profiles||[]),...people,state.profile].filter(p=>p?.id&&ownerIds.has(String(p.id))).map(p=>[String(p.id),p])).values()].sort((a,b)=>label(a).localeCompare(label(b),'fa'));
   host.innerHTML=`<div class="conversation-recipient-head"><h4>انتخاب وظیفه</h4><div class="conversation-task-filters"><label>منبع<select data-task-source><option value="kanban">کانبان</option><option value="archive">آرشیو</option></select></label><label>متولی<select data-task-owner><option value="">همه متولی‌ها</option>${owners.map(p=>`<option value="${esc(p.id)}">${esc(label(p))}</option>`).join('')}</select></label><label>از تاریخ<span class="conversation-date-field"><input type="text" readonly data-task-from placeholder="انتخاب تاریخ شمسی"><button type="button" data-task-calendar="from" aria-label="انتخاب تاریخ شروع">▦</button></span></label><label>تا تاریخ<span class="conversation-date-field"><input type="text" readonly data-task-to placeholder="انتخاب تاریخ شمسی"><button type="button" data-task-calendar="to" aria-label="انتخاب تاریخ پایان">▦</button></span></label><button type="button" class="ghost" data-task-date-clear>حذف بازه</button></div></div><div class="conversation-task-choices"></div>`;
   const palette=taskPalette(tasks.length),taskColors=new Map(tasks.map((t,index)=>[t.id,palette[index]]));
   const paint=()=>{const owner=q('[data-task-owner]',host).value,source=q('[data-task-source]',host).value,from=q('[data-task-from]',host).dataset.iso||'',to=q('[data-task-to]',host).dataset.iso||'';q('.conversation-task-choices',host).innerHTML=tasks.filter(t=>!!t.archived===(source==='archive')&&(!owner||String(t.owner_id||t.created_by)===owner)&&(!from||String(t.start_date||t.created_at||'').slice(0,10)>=from)&&(!to||String(t.start_date||t.created_at||'').slice(0,10)<=to)).map(t=>`<button type="button" class="conversation-task-tile" style="--task-fill:${taskColors.get(t.id).fill};--task-ink:${taskColors.get(t.id).ink}" data-task-choice="${t.id}" aria-label="${esc(t.title)}" data-preview="${esc([t.title,t.description,'متولی: '+ownerName(t),t.status].filter(Boolean).join('\n'))}"><span class="conversation-task-id">${esc(digits(t.legacy_id||t.id))}</span></button>`).join('')||'<div class="conversation-empty">وظیفه‌ای در این بازه وجود ندارد.</div>'};paint();host.querySelectorAll('[data-task-owner],[data-task-source]').forEach(input=>input.addEventListener('change',paint));
   host.querySelectorAll('[data-task-calendar], [data-task-from], [data-task-to]').forEach(button=>button.addEventListener('click',()=>openTaskDate(host,button.dataset.taskCalendar|| (button.hasAttribute('data-task-from')?'from':'to'),paint)));
   q('[data-task-date-clear]',host).onclick=()=>{for(const input of qa('[data-task-from],[data-task-to]',host)){input.value='';delete input.dataset.iso}paint()};
 ...
 async function chooseTask(button,view){
  ...
  try{const [directoryRows,eligibleRows]=await Promise.all([directory(),rpc('chat_task_recipient_ids',{p_task_id:Number(task.id)})]);const eligible=new Set((eligibleRows||[]).map(row=>String(row.user_id)));const people=directoryRows.filter(p=>eligible.has(String(p.id)));if(epoch!==serial||selectedTask!==task)return;
