...
(()=>{
 ...
 async function newConversation(view){const feature=view.id.replace(/View$/,'');if(!can(feature,'create'))return denied(feature,'create');selectedTask=null;window.bamcoChat.close();const host=q('.conversation-stage',view);loading(host);const people=(await directory()).filter(p=>p.id!==state.user.id);if(!host.isConnected)return;
  if(view.id==='taskChatsView'){
   ...
   const tasks=(state.tasks||[]).sort(compareTaskNumbers),ownerIds=new Set(tasks.map(task=>String(task.owner_id||'')).filter(Boolean)),showOwnerFilter=ownerIds.size>1;
   host.innerHTML=`<div class="conversation-recipient-head"><h4>انتخاب وظیفه</h4><p>وظیفه را انتخاب کنید؛ سپس مخاطب گفت‌وگو را مشخص کنید.</p><div class="conversation-task-filters"><label>منبع<select data-task-source><option value="kanban">کانبان</option><option value="archive">آرشیو</option></select></label><label>از تاریخ<input type="date" data-task-from></label><label>تا تاریخ<input type="date" data-task-to></label></div>${showOwnerFilter?`<label class="conversation-owner-filter"><span>متولی</span><select data-task-owner><option value="">همه متولی‌ها</option>${people.filter(p=>ownerIds.has(String(p.id))).map(p=>`<option value="${esc(p.id)}">${esc(label(p))}</option>`).join('')}<option value="${esc(state.user.id)}">وظایف من</option></select></label>`:''}</div><div class="conversation-task-choices"></div>`;
   const palette=taskPalette(tasks.length),taskColors=new Map(tasks.map((t,index)=>[t.id,palette[index]]));
   const paint=()=>{const owner=q('[data-task-owner]',host)?.value,source=q('[data-task-source]',host).value,from=q('[data-task-from]',host).value,to=q('[data-task-to]',host).value; q('.conversation-task-choices',host).innerHTML=tasks.filter(t=>!!t.archived===(source==='archive')&&(!owner||String(t.owner_id)===owner)&&(!from||String(t.start_date||t.created_at||'').slice(0,10)>=from)&&(!to||String(t.start_date||t.created_at||'').slice(0,10)<=to)).map(t=>`<button type="button" class="conversation-task-tile" style="--task-fill:${taskColors.get(t.id).fill};--task-ink:${taskColors.get(t.id).ink}" data-task-choice="${t.id}" aria-label="${esc(t.title)}" data-preview="${esc([t.title,t.description,'متولی: '+ownerName(t),t.status].filter(Boolean).join('\n'))}"><span class="conversation-task-id">${esc(digits(t.legacy_id||t.id))}</span></button>`).join('')||'<div class="conversation-empty">وظیفه‌ای در این بازه وجود ندارد.</div>'};paint();host.querySelectorAll('[data-task-owner],[data-task-source],[data-task-from],[data-task-to]').forEach(input=>input.addEventListener('change',paint));
