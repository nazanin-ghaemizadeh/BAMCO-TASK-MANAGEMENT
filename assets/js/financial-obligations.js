...
(() => {
  ...
  function render() {
    ...
    const commandBar = current
      ? '<div class="invoice-top-command-row bamco-command-bar"><span data-feature-access-suppressed="true" hidden></span><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><button type="button" class="ghost" data-invoice-action="back">بازگشت به کارت‌ها</button><button type="button" class="primary" data-invoice-action="payment">ثبت مرحله پرداخت</button><button type="button" class="ghost" data-invoice-action="edit">ویرایش</button><button type="button" class="danger" data-invoice-action="delete">حذف</button></div>'
