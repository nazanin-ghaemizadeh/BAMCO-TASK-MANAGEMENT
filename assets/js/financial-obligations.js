...
(() => {
  ...
  async function savePayment(event) {
    ...
    try {
      ...
      model.paymentEditor = null; model.paymentFormOpen = false; await load(); render(); notify(paymentId ? 'مرحله پرداخت ویرایش شد؛ مانده و درصد پرداخت به‌روزرسانی شدند.' : 'مرحله پرداخت ثبت شد؛ مانده و درصد پرداخت به‌روزرسانی شدند.');
    } catch (error) { notify(error.message, true); } finally { setBusy(button, false); }
  }
  ...
  function bind() {
    ...
    host.addEventListener('click', event => {
      ...
      if (action === 'payment') { model.paymentEditor = null; model.paymentFormOpen = true; return render(); }
      if (event.target.closest('[data-invoice-close]')) { model.invoiceEditor = null; event.target.closest('dialog')?.close(); return; }
      if (event.target.closest('[data-invoice-cancel]')) { model.paymentEditor = null; model.paymentFormOpen = false; return render(); }
      const dateButton = event.target.closest('[data-invoice-date]'); if (dateButton) return openInvoiceDate(dateButton);
      const stageEdit = event.target.closest('[data-invoice-payment-edit]'); if (stageEdit) { model.paymentEditor = stageEdit.dataset.invoicePaymentEdit; model.paymentFormOpen = true; return render(); }
