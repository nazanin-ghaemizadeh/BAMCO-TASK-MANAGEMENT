...
setInterval(()=>{if(typeof state==='undefined'||!state.profile)return;const identity=JSON.stringify(window.bamcoAuth?.snapshot?.()||state.profile.id);if(boundUser!==identity){boundUser=identity;document.querySelector('#pushSettings')?.remove()}mount();if(!openedNotification&&new URL(location.href).searchParams.has('notification')){openedNotification=true;showView('messages');const url=new URL(location.href);url.searchParams.delete('notification');history.replaceState(null,'',url)}},1500);
})();
