...
(() => {
  ...
  const projectProgress = row => {
    const executable = items(row.id).filter(entry => ['activity', 'task', 'subtask', 'milestone'].includes(entry.item_type));
    return row.progress_override ?? (executable.length ? executable.reduce((sum, entry) => sum + Number(entry.progress || 0), 0) / executable.length : Number(row.progress || 0));
  };
  ...
  function detailMarkup(row) {
    ...
    return `<div class="project-detail-head"><div><h3>${esc(row.title)}</h3><p>${esc(row.description || 'بدون توضیحات')}</p></div><span class="status-badge">${esc(statusText(row.status))}</span></div><div class="project-kpis"><div><b>${fa(Math.round(projectProgress(row)))}٪</b><span>پیشرفت پروژه</span>${progress(projectProgress(row))}</div><div><b>${fa(rows.length)}</b><span>آیتم‌های برنامه</span></div><div><b>${fa(rows.filter(entry => Number(entry.progress) >= 100).length)}</b><span>تکمیل‌شده</span></div><div><b>${fa(deps.length)}</b><span>روابط برنامه</span></div></div><div class="project-dates"><span>شروع برنامه‌ای: ${date(row.planned_start)}</span><span>پایان برنامه‌ای: ${date(row.planned_end)}</span><span>ایجاد: ${dateTime(row.created_at)}</span></div><div class="project-actions project-command-bar"><button type="button" class="primary" data-project-action="item">افزودن آیتم</button><button type="button" class="ghost" data-project-action="dependency">مدیریت روابط</button></div><div class="project-tabs" role="tablist"><button type="button" role="tab" class="${model.detailView === 'gantt' ? 'active' : ''}" data-project-view="gantt">نمای گانت</button><button type="button" role="tab" class="${model.detailView === 'wbs' ? 'active' : ''}" data-project-view="wbs">نمای ساختار شکست</button></div><section class="project-view-panel">${model.detailView === 'gantt' ? ganttMarkup(rows) : workBreakdownMarkup(rows)}</section>`;
  }
  ...
  function workBreakdownMarkup(rows) {
    if (!rows.length) return '<div class="empty">برای شروع، یک فاز، فعالیت یا نقطه عطف اضافه کنید.</div>';
    ...
    return `<div class="wbs-board"><div class="wbs-board-head"><span>ساختار اجرایی پروژه</span><small>برای ویرایش هر بخش، روی همان کارت اقدام کنید.</small></div>${(byParent.get('__top__') || []).map(row => card(row)).join('')}${rows.filter(row => row.parent_item_id && !rows.some(parent => String(parent.id) === String(row.parent_item_id))).map(row => card(row)).join('')}</div>`;
  }
  function ganttMarkup(rows) {
    const dated = rows.filter(row => row.planned_start || row.planned_end); if (!dated.length) return '<div class="empty">برای نمایش گانت، تاریخ برنامه‌ای آیتم‌ها را ثبت کنید.</div>';
    const stamp = value => value ? Date.parse(`${value}T00:00:00Z`) : null, day = 86400000;
    ...
    const count = Math.max(1, Math.min(62, Math.floor((end - start) / day) + 1)), days = Array.from({ length: count }, (_, index) => new Date(start + index * day).toISOString().slice(0, 10));
    return `<div class="project-gantt-wrap"><div class="project-gantt" style="--project-day-count:${count}"><div class="project-gantt-head"><div>آیتم</div><div>زمان‌بندی</div><div class="project-gantt-days">${days.map(value => `<span>${esc(date(value).split('/').slice(-1)[0])}</span>`).join('')}</div></div>${dated.map(row => { const from = stamp(row.planned_start || row.planned_end), to = stamp(row.planned_end || row.planned_start), offset = Math.max(0, Math.floor((from - start) / day)), duration = Math.max(1, Math.floor((to - from) / day) + 1); return `<div class="project-gantt-row"><div><b>${esc(row.title)}</b><small>${esc(itemTypeText(row.item_type))}</small></div><div><span>${date(row.planned_start || row.planned_end)}</span>${row.item_type !== 'milestone' ? `<span>${date(row.planned_end || row.planned_start)}</span>` : '<span>نقطه عطف</span>'}</div><div class="project-gantt-track">${row.item_type === 'milestone' ? `<i class="project-gantt-milestone" style="right:calc(${offset} / var(--project-day-count) * 100% - 7px)" title="${esc(row.title)}"></i>` : `<i class="project-gantt-bar" style="right:calc(${offset} / var(--project-day-count) * 100% + 3px);width:calc(${duration} / var(--project-day-count) * 100% - 6px)" title="${esc(row.title)} — ${fa(Math.round(row.progress || 0))}٪"></i>`}</div></div>`; }).join('')}</div></div>`;
  }
  function itemDialogMarkup(current) {
    ...
    const types = [['phase','فاز'],['activity','فعالیت'],['task','وظیفه اصلی'],['subtask','زیر‌وظیفه'],['milestone','نقطه عطف']], statuses = [['planned','برنامه‌ریزی‌شده'],['in_progress','در حال اجرا'],['waiting','در انتظار'],['completed','تکمیل‌شده']];
    return `<dialog id="projectItemDialog" class="modal enterprise-modal project-item-dialog"><form id="projectItemForm" method="dialog"><input type="hidden" name="project_id" value="${current.id}"><input type="hidden" name="item_id" value="${editing?.id || ''}"><div class="modal-head"><div><h3>${editing ? 'ویرایش آیتم پروژه' : 'افزودن آیتم پروژه'}</h3><p>مسئول آیتم، ایجادکنندهٔ پروژه است.</p></div><button type="button" data-project-close aria-label="بستن">×</button></div><div class="form-grid project-item-form"><label>نوع آیتم<select name="item_type" data-project-item-type>${types.map(([value,label]) => `<option value="${value}" ${editing?.item_type === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label>عنوان<input name="title" value="${esc(editing?.title || '')}" required></label><label class="span-2">آیتم بالادست<select name="parent_item_id"><option value="">در سطح اول ساختار</option>${parents}</select></label><div class="span-2 item-milestone-date ${editing?.item_type === 'milestone' ? '' : 'hidden'}">${dateField('milestone_date', 'تاریخ نقطه عطف', editing?.item_type === 'milestone' ? editing.planned_start : '')}</div><div class="item-range-start ${editing?.item_type === 'milestone' ? 'hidden' : ''}">${dateField('item_planned_start', 'شروع برنامه‌ای', editing?.planned_start)}</div><div class="item-range-end ${editing?.item_type === 'milestone' ? 'hidden' : ''}">${dateField('item_planned_end', 'پایان برنامه‌ای', editing?.planned_end)}</div><label>وضعیت<select name="status">${statuses.map(([value,label]) => `<option value="${value}" ${editing?.status === value || (!editing && value === 'planned') ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label>درصد پیشرفت<input name="progress" type="number" min="0" max="100" value="${esc(editing?.progress ?? 0)}"></label></div><div class="modal-actions"><button type="button" class="ghost" data-project-close>انصراف</button><button type="submit" class="primary">${editing ? 'ذخیره تغییرات' : 'ثبت آیتم'}</button></div></form></dialog>`;
  }
  ...
  async function saveItem(event) {
    ...
    const parentId = form.elements.parent_item_id.value || null; if (id && String(parentId) === String(id)) return notify('یک آیتم نمی‌تواند بالادست خودش باشد.', true);
    ...
    setBusy(button, true); try { if (id) { delete payload.created_by; await update('project_items', `id=eq.${encodeURIComponent(id)}`, payload); notify('آیتم ویرایش شد.'); } else { await insert('project_items', payload); notify('آیتم اضافه شد.'); } model.itemEditor = null; q('#projectItemDialog', root())?.close(); await load(); } catch (error) { notify(error.message, true); } finally { setBusy(button, false); }
  }
  async function saveDependency(event) {
    event.preventDefault(); const form = event.target, button = q('[type=submit]', form), id = form.elements.dependency_id.value, predecessor = Number(form.elements.predecessor_item_id.value), successor = Number(form.elements.successor_item_id.value); if (!predecessor || !successor || predecessor === successor) return notify('پیش‌نیاز و پس‌نیاز باید دو آیتم متفاوت باشند.', true);
