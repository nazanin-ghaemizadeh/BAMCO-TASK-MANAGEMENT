...
(() => {
  ...
  function render() {
    ...
    const commandBar = current ? `<div class="project-top-command-row"><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><button type="button" class="ghost" data-project-action="back">بازگشت به پروژه‌ها</button><button type="button" class="ghost" data-project-action="edit">ویرایش پروژه</button><button type="button" class="danger" data-project-action="delete">حذف پروژه</button><button type="button" class="primary" data-project-action="new">افزودن پروژه</button><button type="button" class="primary" data-project-action="item">افزودن فعالیت</button><button type="button" class="ghost" data-project-action="dependency">مدیریت روابط</button><button type="button" class="ghost ${model.detailView === 'gantt' ? 'active' : ''}" data-project-view="gantt">نمای گانت</button><button type="button" class="ghost ${model.detailView === 'wbs' ? 'active' : ''}" data-project-view="wbs">نمای ساختار شکست</button><input id="projectSearch" type="search" value="${esc(model.search)}" placeholder="جست‌وجوی پروژه…"><button type="button" class="ghost" data-project-action="refresh">تازه‌سازی</button></div>` : `<div class="project-top-command-row"><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><input id="projectSearch" type="search" value="${esc(model.search)}" placeholder="جست‌وجوی پروژه…"><button type="button" class="primary" data-project-action="new">افزودن پروژه</button><button type="button" class="ghost" data-project-action="refresh">تازه‌سازی</button></div>`;
  ...
  function detailMarkup(row) {
    ...
    if (model.detailView === 'wbs') return `<section class="project-full-view"><div class="project-full-view-head"><h3>${esc(row.title)}</h3><span>نمای ساختار شکست</span></div>${workBreakdownMarkup(row, rows)}</section>`;
    if (model.detailView === 'gantt') return `<section class="project-full-view"><div class="project-full-view-head"><h3>${esc(row.title)}</h3><span>نمای گانت</span></div>${ganttMarkup(rows)}</section>`;
  ...
  function workBreakdownMarkup(projectRow, rows) {
    ...
    const node = row => `<div class="wbs-tree-node ${row.item_type}" data-project-item-open="${row.id}" title="برای ویرایش دوبار کلیک کنید"><span>${esc(itemTypeText(row.item_type))}</span><b>${esc(row.title)}</b><small>${row.item_type === 'milestone' ? date(row.planned_start) : `${fa(Math.round(calculatedProgress(row)))}٪ پیشرفت`}</small></div>`;
    const branch = phase => { const descendants = (byParent.get(String(phase.id)) || []).map(child => `<div class="wbs-child-branch">${node(child)}${(byParent.get(String(child.id)) || []).map(grandChild => `<div class="wbs-grandchild">${node(grandChild)}</div>`).join('')}</div>`).join(''); return `<section class="wbs-phase-branch">${node(phase)}<div class="wbs-children">${descendants || '<small class="wbs-empty">فعالیتی تعریف نشده است.</small>'}</div></section>`; };
    const top = byParent.get('__top__') || [], phases = top.filter(row => row.item_type === 'phase'), other = top.filter(row => row.item_type !== 'phase');
    return `<div class="wbs-board wbs-diagram"><div class="wbs-root"><span>پروژه</span><b>${esc(projectRow.title)}</b><small>${fa(Math.round(projectProgress(projectRow)))}٪ پیشرفت</small></div><div class="wbs-phase-grid">${phases.map(branch).join('')}${other.length ? `<section class="wbs-phase-branch"><div class="wbs-tree-node phase"><span>ساختار</span><b>فعالیت‌های بدون فاز</b></div><div class="wbs-children">${other.map(node).join('')}</div></section>` : ''}</div></div>`;
  }
  ...
  function syncItemType(form) { const milestone = form.elements.item_type.value === 'milestone', phase = form.elements.item_type.value === 'phase'; form.querySelector('.item-milestone-date')?.classList.toggle('hidden', !milestone); form.querySelector('.item-range-start')?.classList.toggle('hidden', milestone); form.querySelector('.item-range-end')?.classList.toggle('hidden', milestone); form.querySelector('.item-progress')?.classList.toggle('hidden', phase); form.querySelector('.item-phase-progress')?.classList.toggle('hidden', !phase); }
