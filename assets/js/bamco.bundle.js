...
(() => {
  ...
  function workBreakdownMarkup(projectRow, rows) {
    ...
    const branchCount = phases.length + (other.length ? 1 : 0);
    return `<div class="wbs-direct"><div class="wbs-root"><b>${esc(projectRow.title)}</b><small>${fa(Math.round(projectProgress(projectRow)))}٪ پیشرفت</small></div><div class="wbs-phase-grid ${branchCount > 1 ? 'has-multiple-phases' : ''}" style="--phase-count:${Math.max(1,branchCount)}">${phases.map(phase => `<section class="wbs-phase-branch" style="--phase-color:${phaseMeta(phase, rows).color}">${branch(phase)}</section>`).join('')}${other.length ? `<section class="wbs-phase-branch wbs-unphased">${other.map(row => branch(row)).join('')}</section>` : ''}</div></div>`;
  }
  function ganttMarkup(rows) {
    ...
    const relationSvg = projectDependencies(rows).filter(dep => pos.has(String(dep.predecessor_item_id)) && pos.has(String(dep.successor_item_id))).map(dep => { const from = item(dep.predecessor_item_id), to = item(dep.successor_item_id), kind = dep.dependency_type || 'FS', map = { FS: ['end','start'], SS: ['start','start'], FF: ['end','end'], SF: ['start','end'] }[kind] || ['end','start'], x1 = x(from, map[0]), x2 = x(to, map[1]), y1 = headerH + pos.get(String(from.id)) * rowH + rowH / 2, y2 = headerH + pos.get(String(to.id)) * rowH + rowH / 2, lane = x1 + (x2 >= x1 ? 18 : -18); return `<path d="M ${x1} ${y1} H ${lane} V ${y2} H ${x2}" class="gantt-dependency-line" marker-end="url(#ganttArrow)"><title>${kind} · ${esc(from.title)} ← ${esc(to.title)}</title></path>`; }).join('');
