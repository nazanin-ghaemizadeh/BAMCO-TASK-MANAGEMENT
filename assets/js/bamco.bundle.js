...
let sentRows=[],sentLoading=null,sentLoaded=false;
...
function sentBody({loading=false,error=''}={}){const filtered=sentFiltered();if(loading)return '<tr><td colspan="10" class="empty workspace-loading" role="status">در حال دریافت سابقه پیام‌ها…</td></tr>';if(error)return `<tr><td colspan="10" class="empty"><div class="workspace-error" role="alert"><b>سابقه پیام‌ها دریافت نشد.</b><p>${esc(error)}</p><button type="button" class="ghost" data-sent-retry>تلاش دوباره</button></div></td></tr>`;return filtered.length?filtered.map((r,i)=>`<tr data-sent-key="${esc(r.log_key)}"><td>${faNum(filtered.length-i)}</td><td>${esc(SOURCE_LABEL[r.source_type]||r.source_type||'—')}</td><td>${esc(r.sender_name||'سامانه')}</td><td>${esc(r.recipient_name||'—')}</td><td>${esc(r.subject||'—')}</td><td>${esc(SENT_CHANNEL[r.channel]||r.channel||'—')}</td><td><span class="sent-status ${esc(r.delivery_status||'')}">${esc(SENT_STATUS[r.delivery_status]||r.delivery_status||'—')}</span></td><td>${esc(sentDate(r.sent_at))}</td><td>${faNum(r.attempt_count||0)}</td><td><button type="button" class="ghost" data-sent-detail="${esc(r.log_key)}">مشاهده</button></td></tr>`).join(''):'<tr><td colspan="10" class="empty">رکوردی مطابق فیلترها وجود ندارد.</td></tr>'}
function ensureSentShell(){const view=ensureSentView();if(!q('.sent-workspace',view))view.innerHTML='<div class="panel workspace-panel sent-workspace"><div class="panel-head"><div><h3>پیام‌های ارسال‌شده</h3><small>فقط گزارش وضعیت امور و یادآوری‌ها</small></div></div><div class="sent-command-row bamco-command-bar"><button type="button" class="ghost content-back">بازگشت به خانه</button><button type="button" class="ghost" data-sent-export>خروجی اکسل</button><button type="button" class="ghost" data-sent-refresh>تازه‌سازی</button><button type="button" class="ghost hidden" data-sent-access data-feature-access-control="local">مدیریت دسترسی</button></div><div class="table-wrap"><table class="workspace-table sent-table"><thead><tr><th>ردیف</th><th>نوع</th><th>فرستنده</th><th>گیرنده</th><th>موضوع</th><th>کانال</th><th>وضعیت</th><th>زمان ارسال</th><th>تلاش</th><th>جزئیات</th></tr></thead><tbody>'+sentBody({loading:true})+'</tbody></table></div></div>';bindSentControls(view);syncSentAccess();return view}
