...
(() => {
  'use strict';
  const E = window.bamcoEnterprise; if (!E) return;
  const { q, esc, fa, money, date, fetchRows, insert, insertMinimal, setBusy, notify, statusText } = E;
  const model = { invoices: [], payments: [], selected: null, search: '' };
  let loadVersion = 0;
  ...
  const dateField = (name, label) => `<label>${label}<span class="enterprise-date-field"><input name="${name}_jalali" class="jalali-input" readonly placeholder="۱۴۰۵/۰۱/۰۱"><button type="button" class="ghost bamco-icon-button" data-invoice-date="${name}" aria-label="${label}">▦</button><input name="${name}" type="hidden"></span></label>`;
  function render() {
    ...
    host.innerHTML = `<div class="feature-toolbar enterprise-toolbar"><div><h3>صورتحساب‌ها و تعهدات مالی</h3><p>تعهد مالی، پرداخت مرحله‌ای، سررسید و رسیدها در یک منبع مرکزی ثبت می‌شوند.</p></div><div class="feature-toolbar-actions"><input id="invoiceSearch" type="search" value="${esc(model.search)}" placeholder="شماره، عنوان یا شرکت/پیمانکار…"><button type="button" class="primary" data-invoice-action="new">صورتحساب جدید</button><button type="button" class="ghost" data-invoice-action="refresh">تازه‌سازی</button></div></div><div class="enterprise-grid invoice-grid ${current ? 'detail-open' : ''}"><section class="panel"><div class="panel-head"><h3>لیست صورتحساب‌ها</h3><span class="enterprise-count">${fa(visible.length)} مورد</span></div><div class="enterprise-card-list">${visible.length ? visible.map(item => `<button type="button" class="enterprise-list-card ${String(model.selected) === String(item.id) ? 'active' : ''}" data-invoice-select="${item.id}"><span><strong>${esc(item.title)}</strong><small>${esc(item.invoice_number)} · ${esc(item.company_name || item.account_party)}</small></span><span><b>${money(paid(item.id), item.currency === 'IRR' ? 'ریال' : item.currency)}</b><small>${esc(statusText(item.status))}</small></span></button>`).join('') : '<div class="empty">صورتحسابی ثبت نشده است.</div>'}</div></section><section class="panel invoice-detail">${current ? `<button type="button" class="ghost enterprise-detail-back" data-invoice-action="back">بازگشت به صورتحساب‌ها</button>` + detailMarkup(current) : '<div class="enterprise-empty"><b>صورتحسابی انتخاب نشده است.</b><span>برای شروع یک تعهد مالی ثبت کنید.</span></div>'}</section></div><dialog id="invoiceDialog" class="modal enterprise-modal"><form id="invoiceForm" method="dialog"><div class="modal-head"><div><h3>ثبت صورتحساب</h3></div><button type="button" data-invoice-close>×</button></div><div class="form-grid"><label>شماره صورتحساب<input name="invoice_number" dir="rtl" required></label><label>عنوان<input name="title" required></label><label>شرکت/پیمانکار<input name="company_name" required></label><label>ارز<select name="currency"><option value="IRR">ریال</option><option value="IRT">تومان</option><option value="USD">دلار آمریکا</option><option value="EUR">یورو</option></select></label><label>مبلغ کل<input name="total_amount" type="number" min="0" step="0.01" required></label>${dateField('due_date', 'تاریخ سررسید')}<label class="span-2">توضیحات<textarea name="description" rows="3"></textarea></label></div><div class="modal-actions"><button type="button" class="ghost" data-invoice-close>انصراف</button><button type="submit" class="primary">ثبت صورتحساب</button></div></form></dialog>`; bind();
  }
  function detailMarkup(item) { const rows = payments(item.id); const balance = Math.max(0, Number(item.total_amount || 0) - paid(item.id)); return `<div class="project-detail-head"><div><span class="enterprise-eyebrow">${esc(item.invoice_number)}</span><h3>${esc(item.title)}</h3><p>${esc(item.company_name || item.account_party)}</p></div><span class="status-badge">${esc(statusText(item.status))}</span></div><div class="invoice-total"><div><b>${money(item.total_amount, item.currency === 'IRR' ? 'ریال' : item.currency)}</b><span>مبلغ کل</span></div><div><b>${money(paid(item.id), item.currency === 'IRR' ? 'ریال' : item.currency)}</b><span>پرداخت‌شده</span></div><div><b>${money(balance, item.currency === 'IRR' ? 'ریال' : item.currency)}</b><span>باقی‌مانده</span></div><div><b>${fa(Math.round(percent(item)))}٪</b><span>درصد پرداخت</span></div></div>${E.progress(percent(item))}<div class="invoice-meta"><span>سررسید: ${date(item.due_date)}</span></div><div class="project-actions"><button type="button" class="primary" data-invoice-action="payment">ثبت مرحله پرداخت</button></div><section class="invoice-payments"><div class="panel-head"><h4>پرداخت‌های مرحله‌ای</h4><small>${fa(rows.length)} مرحله</small></div>${rows.length ? rows.map(row => `<article class="payment-row"><b>مرحله ${fa(row.sequence_no)}</b><span>${money(row.amount, item.currency === 'IRR' ? 'ریال' : item.currency)}</span><span>${esc(statusText(row.status))}</span><span>برنامه‌ای: ${date(row.planned_date)}</span><span>واقعی: ${date(row.paid_date)}</span></article>`).join('') : '<div class="empty">مرحله پرداختی ثبت نشده است.</div>'}</section><form id="invoicePaymentForm" class="enterprise-inline-form hidden"><input type="hidden" name="invoice_id" value="${item.id}"><label>شماره مرحله<input name="sequence_no" type="number" min="1" value="${rows.length + 1}"></label><label>مبلغ<input name="amount" type="number" min="0" step="0.01" required></label><label>تاریخ برنامه‌ای<input name="planned_date" type="date"></label><label>وضعیت<select name="status"><option value="planned">برنامه‌ریزی‌شده</option><option value="paid">پرداخت‌شده</option></select></label><label>تاریخ واقعی<input name="paid_date" type="date"></label><label>شماره پیگیری<input name="tracking_no"></label><div><button type="submit" class="primary">ثبت پرداخت</button><button type="button" class="ghost" data-invoice-cancel>انصراف</button></div></form>`; }
  async function saveInvoice(event) { event.preventDefault(); const form = event.target, button = q('[type=submit]', form), company = form.elements.company_name.value.trim(), invoiceNumber = form.elements.invoice_number.value.trim(); setBusy(button, true); try { await (insertMinimal || insert)('invoices', { invoice_number: invoiceNumber, title: form.elements.title.value.trim(), account_party: company, company_name: company, currency: form.elements.currency.value, total_amount: Number(form.elements.total_amount.value || 0), due_date: form.elements.due_date.value || null, description: form.elements.description.value.trim() || null, created_by: state.user.id, follow_up_owner_id: state.user.id }); q('#invoiceDialog',root()).close(); await load(); model.selected = model.invoices.find(item => item.invoice_number === invoiceNumber)?.id || null; render(); notify('صورتحساب ثبت شد.'); } catch (error) { notify(error.message, true); } finally { setBusy(button, false); } }
  ...
  function bind() { const host = root(); if (!host || host.dataset.bound === '1') return; host.dataset.bound = '1'; host.addEventListener('input', event => { if (event.target.id === 'invoiceSearch') { model.search = event.target.value; render(); q('#invoiceSearch')?.focus(); } }); host.addEventListener('click', event => { const action = event.target.closest('[data-invoice-action]')?.dataset.invoiceAction; if (action === 'new') { const dialog=q('#invoiceDialog',host); if(dialog&&!dialog.open)dialog.showModal(); } if (action === 'refresh') void load(); if (action === 'back') { model.selected=null; render(); } if (action === 'payment') q('#invoicePaymentForm')?.classList.remove('hidden'); if (event.target.closest('[data-invoice-close]')) event.target.closest('dialog')?.close(); if (event.target.closest('[data-invoice-cancel]')) event.target.closest('form')?.classList.add('hidden'); const dateButton = event.target.closest('[data-invoice-date]'); if (dateButton) openInvoiceDate(dateButton); const select = event.target.closest('[data-invoice-select]'); if (select) { model.selected = select.dataset.invoiceSelect; render(); } }); host.addEventListener('submit', event => { if (event.target.id === 'invoiceForm') void saveInvoice(event); if (event.target.id === 'invoicePaymentForm') void savePayment(event); }); }
  async function load() { if (!root() || !state.profile) return; const request = ++loadVersion; try { const [invoices, payments] = await Promise.all([fetchRows('invoices', 'select=*&order=updated_at.desc,id.desc'), fetchRows('invoice_payments', 'select=*&order=sequence_no.asc')]); if (request !== loadVersion) return; Object.assign(model, { invoices, payments }); if (model.selected && !invoice(model.selected)) model.selected = null; render(); } catch (error) { if (request === loadVersion && root()) root().innerHTML = `<div class="panel enterprise-error">${esc(error.message)}</div>`; } }
  function boot() {
    if (!root()) return;
    render();
...
async function access(){const layer=window.BamcoAccess;if(state?.token&&!layer?.isReady?.())await layer?.refresh?.();const yes=!!state?.token&&can('view'),err=q('#lettersError');if(err)err.textContent=yes?'':'دسترسی شما به نامه‌ها فعال نیست.';q('#lettersAccess')?.classList.toggle('hidden',!can('manage_access'));q('#addLetter')?.toggleAttribute('disabled',!can('create'));if(!yes&&state?.view==='letters')window.bamcoShowHome?.();return yes}
...
function syncActions(){const ids=selectionIds(),row=ids.length===1?rows.find(r=>String(r.id)===ids[0]):null,one=!!row;q('#letterEditSelected')?.toggleAttribute('disabled',!one||!can('edit'));q('#letterDownloadSelected')?.toggleAttribute('disabled',!one||!row?.storage_path||!can('export'));q('#letterDeleteSelected')?.toggleAttribute('disabled',!one||!can('delete'))}
function render(){
 ...
 if(liveTable&&!liveTable.dataset.actionsBound){liveTable.dataset.actionsBound='true';liveTable.addEventListener('click',async event=>{const button=event.target.closest('[data-letter-edit],[data-letter-download],[data-letter-delete]');if(!button)return;const row=rows.find(item=>String(item.id)===String(button.dataset.letterId));if(!row)return;if(button.hasAttribute('data-letter-edit'))return form(row);if(button.hasAttribute('data-letter-download')){if(!can('export'))return deny('export');const result=await edge({action:'download',id:row.id}),anchor=document.createElement('a');anchor.href=result.url;anchor.rel='noopener';anchor.click();return}if(!can('delete'))return deny('delete');if(await ask('نامه و فایل ضمیمه آن حذف شود؟')){await edge({action:'delete',id:row.id,version:row.version});await load();toast('نامه حذف شد.')}})}
...
function switchDirection(next){if(!['incoming','outgoing'].includes(next))return;direction=next;clearSelection();qa('[data-letter-mode]',q('#lettersView')).forEach(button=>{const active=button.dataset.letterMode===next;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active))});render();void load()}
...
function form(row=null){if(!can(row?'edit':'create'))return deny(row?'edit':'create');let d=q('#letterDialog');if(!d){d=document.createElement('dialog');d.id='letterDialog';d.className='letters-dialog bamco-dialog';document.body.append(d)}const party=direction==='incoming'?'فرستنده':'گیرنده';d.innerHTML=`<form><div class="modal-head"><div><h3>${row?'ویرایش':'افزودن'} نامه ${direction==='incoming'?'ورودی':'خروجی'}</h3><p>اطلاعات و فایل نامه را ثبت کنید.</p></div><button type="button" data-close>×</button></div><div class="form-grid"><label>شماره نامه<input name="letter_number" required maxlength="120" dir="ltr" value="${esc(row?.letter_number||'')}"></label><label>تاریخ نامه<span class="letter-date-field"><input name="letter_date" class="jalali-input" readonly required value="${esc(row?.letter_date||'')}"><button type="button" class="ghost bamco-icon-button" data-letter-calendar>▦</button></span></label><label>${party}<input name="recipient" required maxlength="300" value="${esc(row?.recipient||'')}"></label><label>موضوع نامه<input name="subject" required maxlength="1000" value="${esc(row?.subject||'')}"></label><label class="span-2 letter-file-picker"><span>فایل نامه یا سند</span><input type="file" name="file" accept=".pdf,.docx,.png,.jpg,.jpeg" ${row?'':'required'}><span class="letter-file-choice"><span class="ghost">انتخاب فایل</span><b data-letter-file-name>${esc(row?.file_name||'فایلی انتخاب نشده است')}</b></span><small>حداکثر حجم فایل: ۲۵ مگابایت</small></label></div><p class="letter-error" role="alert"></p><div class="modal-actions"><button class="ghost" type="button" data-close>انصراف</button><button class="primary" type="submit">ذخیره</button></div></form>`;const f=q('form',d);qa('[data-close]',d).forEach(b=>b.onclick=()=>{if(!saving)d.close()});q('[data-letter-calendar]',f).onclick=()=>openCalendar(f.elements.letter_date);f.elements.file.onchange=()=>q('[data-letter-file-name]',f).textContent=f.elements.file.files[0]?.name||'فایلی انتخاب نشده است';f.onsubmit=async e=>{e.preventDefault();if(saving||!can(row?'edit':'create'))return deny(row?'edit':'create');saving=true;const b=q('[type=submit]',f);b.disabled=true;const fd=new FormData(f);fd.set('action','save');fd.set('direction',direction);if(row){fd.set('id',row.id);fd.set('version',row.version)}try{await edge(fd);d.close();toast('نامه ذخیره شد.');await load()}catch(err){q('.letter-error',f).textContent=err.message}finally{saving=false;b.disabled=false}};d.showModal()}
async function manageAccess(){return window.bamcoAccessEditor?.open?.({featureKey:'letters',title:'دسترسی به نامه‌ها'})}
function init(){
 const view=q('#lettersView');if(!view)return;try{titles.letters='نامه‌ها'}catch{}
 view.innerHTML='<section id="lettersPanel" class="letters-panel"><div class="letter-toolbar"><button class="content-back ghost">بازگشت به خانه</button><button class="primary" id="addLetter">افزودن نامه</button><button class="ghost" id="refreshLetters">تازه‌سازی</button><button class="ghost" id="letterEditSelected" disabled>ویرایش</button><button class="ghost" id="letterDownloadSelected" disabled>دانلود</button><button class="danger" id="letterDeleteSelected" disabled>حذف</button><button class="ghost hidden" id="lettersAccess">مدیریت دسترسی</button><input type="search" id="lettersSearch" placeholder="جست‌وجوی نامه…"></div><div class="letters-tabs" role="tablist" aria-label="نوع نامه"><button type="button" role="tab" data-letter-mode="incoming">نامه‌های ورودی</button><button type="button" role="tab" data-letter-mode="outgoing">نامه‌های خروجی</button></div><div class="letters-heading"><h3 id="lettersModeTitle">نامه‌ها</h3></div><p id="lettersError" class="letter-error"></p><div id="lettersBody"></div></section>';
 q('.content-back',view).onclick=()=>window.bamcoShowHome?.();
 qa('[data-letter-mode]',view).forEach(button=>button.onclick=()=>switchDirection(button.dataset.letterMode));
 q('#addLetter').onclick=()=>{if(!can('create'))return deny('create');return form()};
 q('#refreshLetters').onclick=()=>void load();
 q('#letterEditSelected').onclick=()=>{const r=selectedRow();if(r)form(r)};
 q('#letterDownloadSelected').onclick=async()=>{const r=selectedRow();if(!r||!can('export'))return deny('export');const x=await edge({action:'download',id:r.id}),a=document.createElement('a');a.href=x.url;a.rel='noopener';a.click()};
 q('#letterDeleteSelected').onclick=async()=>{const r=selectedRow();if(!r||!can('delete'))return deny('delete');if(await ask('نامه و فایل ضمیمه آن حذف شود؟')){await edge({action:'delete',id:r.id,version:r.version});clearSelection();await load();toast('نامه حذف شد.')}};
 q('#lettersAccess').onclick=manageAccess;
 q('#lettersSearch').oninput=e=>{query=e.target.value.trim().toLowerCase();render()};
 view.addEventListener('bamco-selection-change',syncActions);
 let shown=false;new MutationObserver(()=>{const now=!view.classList.contains('hidden');if(now&&!shown){shown=true;switchDirection(direction||'incoming')}else if(!now&&shown){shown=false;clearSelection()}}).observe(view,{attributes:true,attributeFilter:['class']});
 void access();syncActions();
}
...
const appView=q('#appView');if(appView)new MutationObserver(()=>{if(!appView.classList.contains('hidden'))void access()}).observe(appView,{attributes:true,attributeFilter:['class']});
window.addEventListener('bamco:feature-access-changed',()=>{void access();render();syncActions()});
})();
;
