...
function init(){
 ...
 q('#switchLetters').onclick=choose;
 q('#addLetter').onclick=()=>{if(!can('create'))return deny('create');return direction?form():choose()};
 ...
 let shown=false;new MutationObserver(()=>{const now=!view.classList.contains('hidden');if(now&&!shown){shown=true;if(!direction){if(q('#appView')){direction='outgoing';void load()}else choose()}else void load()}else if(!now&&shown){shown=false;clearSelection()}}).observe(view,{attributes:true,attributeFilter:['class']});
