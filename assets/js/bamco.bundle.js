...
(()=>{
  ...
  const can=(scope,action='view')=>window.BamcoAccess?.can?.(scope,action)===true;
  const deny=(scope,action='view')=>window.BamcoAccess?.denied?.(scope,action);
  ...
  const normalize=v=>latinDigits(v).replace(/ي/g,'ی').replace(/ك/g,'ک').replace(/\u200c/g,' ').replace(/\s+/g,' ').trim();
  const displayCell=(key,value)=>key==='vehicle_type'?`<span class="vehicle-type-latin" dir="ltr">${esc(value??'—')}</span>`:`<span class="vehicle-persian-value">${esc(fan(value??'—'))}</span>`;
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='vehicleModuleStyles';s.textContent=`
      ...
      #appView .vehicle-data-table td{border:1px solid #d1ddd8!important;padding:8px!important;text-align:right!important;vertical-align:top!important;white-space:normal!important;overflow-wrap:anywhere!important;line-height:1.7!important;font-family:"B Nazanin",BNazanin,Tahoma,sans-serif!important;direction:rtl!important}
      #appView .vehicle-persian-value{font-family:"B Nazanin",BNazanin,Tahoma,sans-serif!important;direction:rtl!important;unicode-bidi:isolate!important}
      #appView .vehicle-type-latin,#vehicleRecordDialog .vehicle-type-latin{font-family:"Times New Roman",Times,serif!important;direction:ltr!important;text-align:left!important;unicode-bidi:isolate!important}
  ...
  async function uploadBlankTemplate(e){const file=e.target.files?.[0];e.target.value='';if(!file)return;if(!can('vehiclePermanent','edit'))return deny('vehiclePermanent','edit');if(file.type!=='application/pdf'||file.size>20*1024*1024){toast('فرم خام باید PDF و حداکثر ۲۰ مگابایت باشد.',true);return}try{const dataUrl=await readDataUrl(file),fileBase64=dataUrl.split(',')[1]||'';await update('vehicle_form_templates','template_key=eq.permanent_handover_blank',{title:'فرم خام تحویل و تحول خودرو',filename:file.name,mime_type:file.type,file_base64:fileBase64,updated_at:new Date().toISOString()});toast('فرم خام جدید ثبت شد.')}catch(err){toast(err.message,true)}}
  async function downloadBlankTemplate(){if(!can('vehiclePermanent','export'))return deny('vehiclePermanent','export');try{const rows=await selectAll('vehicle_form_templates','select=*&template_key=eq.permanent_handover_blank&limit=1'),row=rows[0];if(!row?.file_base64){location.href='assets/forms/vehicle-permanent-blank.pdf';return}const bytes=Uint8Array.from(atob(row.file_base64),c=>c.charCodeAt(0)),url=URL.createObjectURL(new Blob([bytes],{type:row.mime_type||'application/pdf'})),a=document.createElement('a');a.href=url;a.download=row.filename||'فرم خام تحویل خودرو.pdf';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(err){toast(err.message,true)}}
  ...
  async function saveRecord(e){
    ...
    if(!can(scope,wasEditing?'edit':'create'))return deny(scope,wasEditing?'edit':'create');
  ...
  function load(scope,permission){const c=CFG[scope],s=data[scope];if(!state?.token)return Promise.resolve();if(s.pending)return s.pending;const token=state.token;s.loading=true;s.error='';s.denied=false;render(scope);const run=(async()=>{try{const allowed=typeof permission==='boolean'?permission:await permitted(scope);if(state.token!==token)return;if(!allowed){s.rows=[];s.selected.clear();s.denied=true;s.loaded=false;return}// Permanent handovers are a register: the most recently registered row is always first.
      const order=scope==='vehiclePermanent'?'id.desc':'id.asc';s.rows=await selectAll(c.table,`select=*&order=${order}`);if(state.token!==token)return;const valid=new Set(s.rows.map(r=>String(r.id)));s.selected=new Set([...s.selected].filter(id=>valid.has(id)));s.loaded=true}catch(err){if(state.token===token){s.error=err?.message||'دریافت اطلاعات انجام نشد.';s.loaded=false}}finally{if(s.pending===run)s.pending=null;if(state.token===token){s.loading=false;render(scope)}}})();s.pending=run;return run}
  function displayId(scope,row){const index=data[scope].rows.findIndex(item=>String(item.id)===String(row.id));if(index<0)return'';return scope==='vehiclePermanent'?data[scope].rows.length-index:index+1}
  ...
  function render(scope){
    ...
    const formActions=row=>row.form_path?`<div class="vehicle-form-actions"><button class="vehicle-form-action" type="button" data-form-action="preview" data-row-id="${esc(row.id)}" title="پیش‌نمایش فرم" aria-label="پیش‌نمایش فرم">${previewIcon}</button><button class="vehicle-form-action" type="button" data-form-action="download" data-row-id="${esc(row.id)}" title="دانلود فرم" aria-label="دانلود فرم"${can(scope,'export')?'':' disabled'}>${downloadIcon}</button></div>`:'—';
    const rows=visibleRows(scope),stateMessage=s.error?`<div>${esc(s.error)}</div><button type="button" class="ghost" data-vehicle-retry="${scope}">تلاش دوباره</button>`:s.denied?'دسترسی به این بخش برای شما فعال نیست.':!s.loaded?'در حال بارگذاری اطلاعات…':rows.length?null:'موردی برای نمایش وجود ندارد.',stateClass=s.loading?' vehicle-loading-state':'';q('tbody',view).innerHTML=rows.length?rows.map(({row})=>{const selected=s.selected.has(String(row.id)),shownId=displayId(scope,row),pick=`<td class="vehicle-pick-cell"><input class="task-pick vehicle-pick" type="checkbox" value="${row.id}" ${selected?'checked':''} aria-label="انتخاب ردیف ${fan(shownId)}"></td>`;return `<tr data-id="${row.id}" class="${selected?'vehicle-selected task-selected':''}" aria-selected="${selected}" tabindex="0"><td><span class="vehicle-persian-value">${fan(shownId)}</span></td>${c.fields.map(([key])=>`<td>${displayCell(key,row[key])}</td>`).join('')}${c.form?`<td class="vehicle-form-status">${formActions(row)}</td>`:''}</tr>`}).join(''):`<tr><td class="empty${stateClass}" colspan="${labels.length}" ${s.loading?'aria-busy="true"':''}>${stateMessage||''}</td></tr>`;
  ...
  function updateButtons(scope){const view=q(`#${CFG[scope].view}`),count=data[scope].selected.size,single=count===1;q('.vehicle-access-button',view)?.classList.toggle('hidden',!can(scope,'manage_access'));q('.vehicle-import',view).disabled=!can(scope,'create');q('.vehicle-export',view).disabled=!can(scope,'export');q('.vehicle-add',view).disabled=!can(scope,'create');q('.vehicle-edit',view).disabled=!count||!can(scope,'edit');q('.vehicle-delete',view).disabled=!count||!can(scope,'delete');if(CFG[scope].form){q('.vehicle-upload',view).disabled=!single||!can(scope,'edit');q('.vehicle-template-upload',view).disabled=!can(scope,'edit');q('.vehicle-blank',view).disabled=!can(scope,'export')}}
  ...
  function chooseForm(scope,mode='toolbar'){if(!can(scope,'edit'))return deny(scope,'edit');uploadScope=scope;formInputMode=mode;q('#vehicleFormInput').value='';q('#vehicleFormInput').click()}
  ...
  function bind(scope){
    ...
    q('.vehicle-add',view).onclick=()=>{if(!can(scope,'create'))return deny(scope,'create');window.bamcoSelection.clear('#'+c.view+' tbody');openEditor(scope)};
    q('.vehicle-edit',view).onclick=()=>{if(!can(scope,'edit'))return deny(scope,'edit');if(data[scope].selected.size!==1)return toast('برای ویرایش فقط یک ردیف را انتخاب کنید.',true);openEditor(scope)};
    q('.vehicle-delete',view).onclick=()=>removeRow(scope);
    const search=q('.vehicle-search',view),toggle=q('.vehicle-search-toggle',view);
    toggle.onclick=()=>{const open=search.classList.toggle('search-open');toggle.classList.toggle('active',open);if(open)setTimeout(()=>search.focus(),20)};
    search.onkeydown=e=>{if(e.key==='Escape'){search.classList.remove('search-open');toggle.classList.remove('active');search.blur()}};
    search.oninput=e=>{data[scope].query=e.target.value;render(scope)};
    q('thead',view).onchange=e=>{if(e.target.matches('[data-filter]')){data[scope].filters[Number(e.target.dataset.filter)]=e.target.value;render(scope)}};
    q('tbody',view).onclick=e=>{const retry=e.target.closest('[data-vehicle-retry]');if(retry){e.preventDefault();e.stopPropagation();void load(scope);return}const row=e.target.closest('tr[data-id]');if(!row)return;const action=e.target.closest('[data-form-action]');if(action){e.preventDefault();e.stopPropagation();action.dataset.formAction==='preview'?previewForm(scope,action.dataset.rowId):downloadForm(scope,action.dataset.rowId);return}const id=String(row.dataset.id);if(data[scope].selected.has(id))data[scope].selected.delete(id);else data[scope].selected.add(id);render(scope)};
    if(c.form){q('.vehicle-upload',view).onclick=()=>chooseForm(scope);q('.vehicle-template-upload',view).onclick=()=>{if(!can(scope,'edit'))return deny(scope,'edit');q('#vehicleTemplateInput').value='';q('#vehicleTemplateInput').click()};q('.vehicle-blank',view).onclick=downloadBlankTemplate}
    load(scope);
  }
  ...
  async function permitted(scope,action='view'){if(!state?.token)return false;const access=window.BamcoAccess;if(!access?.isReady?.())await access?.refresh?.();return can(scope,action)}
  let accessPending=null;
  async function refreshAccess(){if(accessPending)return accessPending;const token=state?.token;accessPending=(async()=>{const results=await Promise.all(Object.keys(CFG).map(async scope=>[scope,await permitted(scope)]));for(const [scope,allowed] of results){if(state?.token!==token)return;const view=q('#'+CFG[scope].view);q('.vehicle-access-button',view)?.classList.toggle('hidden',!can(scope,'manage_access'));if(!allowed){data[scope].rows=[];data[scope].selected.clear();data[scope].loaded=false;data[scope].denied=true;data[scope].loading=false;render(scope);if(state?.view===scope){q('#vehicleRecordDialog')?.close();window.bamcoShowHome?.()}}else{data[scope].denied=false;updateButtons(scope);if(!data[scope].loaded&&!data[scope].pending)void load(scope,true)}}})().finally(()=>accessPending=null);return accessPending}
  function init(){if(ready)return;const p=q('#vehiclePermanentView'),t=q('#vehicleTemporaryView');if(!p||!t)return;ready=true;addStyles();ensureInputs();bind('vehiclePermanent');bind('vehicleTemporary');void refreshAccess();const app=q('#appView');if(app)new MutationObserver(()=>void refreshAccess()).observe(app,{attributes:true,attributeFilter:['class']});window.addEventListener('bamco:feature-access-changed',()=>void refreshAccess());document.addEventListener('click',e=>{const b=e.target.closest('#nav button[data-view="vehiclePermanent"],#nav button[data-view="vehicleTemporary"]');if(b)setTimeout(()=>load(b.dataset.view),0)},true)}
...
const has=action=>window.BamcoAccess?.can?.('pettyCash',action||'view')===true;
const denied=action=>window.BamcoAccess?.denied?.('pettyCash',action||'view');
async function ensureAccess(action='view'){const access=window.BamcoAccess;if(state?.token&&!access?.isReady?.())await access?.refresh?.();return !!state?.token&&has(action)}
async function load(){if(loadPending)return loadPending;if(!await ensureAccess())return;busy=true;loadPending=(async()=>{try{[entries,attachments]=await Promise.all([selectAll('petty_cash_entries','select=*&order=event_date.desc,entry_no.desc'),selectAll('petty_cash_attachments','select=*&order=created_at.desc')]);loaded=true;render()}catch(e){q('#pettyCashError').textContent=e.message}finally{busy=false;loadPending=null}})();return loadPending}
...
/* source: assets/js/personal-workspace.js */
...
const REMOVED=new Set(['requestReport','responseTracking','responseReport']);
const NEW=[['accessMatrix','دسترسی‌ها','people'],['loginActivity','ورود و خروج','people'],['activeSessions','نشست‌های فعال','people'],['groupChat','گفت‌وگوی عمومی و گروه‌ها','conversations'],['directMessages','گفت‌وگوی خصوصی','conversations'],['taskChats','گفت‌وگوی مرتبط با وظیفه','conversations'],['performanceReport','گزارش عملکرد','reports'],['systemOptions','وضعیت‌ها و اولویت‌ها','configuration']];
const catalog=window.BamcoNavigationCatalog;
if(!catalog)return;
...
function install(){if(installed)return;installed=true;purgeRemoved();syncFeatureControls();bind();window.addEventListener('bamco:feature-access-changed',syncFeatureControls);const nav=q('#nav'),workspace=q('.workspace');if(nav||workspace)new MutationObserver(records=>{if(records.some(r=>[...r.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('#requestReportView,#responseTrackingView,#responseReportView,[data-view="requestReport"],[data-view="responseTracking"],[data-view="responseReport"]')||n.querySelector?.('#requestReportView,#responseTrackingView,#responseReportView,[data-view="requestReport"],[data-view="responseTracking"],[data-view="responseReport"]')))))purgeRemoved()}).observe(q('#appView')||document.body,{childList:true,subtree:true});window.bamcoProductionRuntime={navigate,render}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
})();
;
...
const q=(s,r=document)=>r.querySelector(s),qa=(s,r=document)=>[...r.querySelectorAll(s)],dt=v=>v&&Number.isFinite(Date.parse(v))?new Date(v).toLocaleString('fa-IR'):'—',word=v=>labels[v]||v||'—',requestWord=row=>window.bamcoRequestTypeLabel?.(row)||word(row?.request_type);
...
const titles={sentMessages:'پیام‌های ارسال‌شده',loginActivity:'ورود و خروج کاربران',activeSessions:'نشست‌های فعال',loginReport:'گزارش ورود و خروج',performanceReport:'گزارش عملکرد',messageReport:'گزارش پیام‌ها',requestReport:'گزارش درخواست‌ها',systemOptions:'وضعیت‌ها و اولویت‌ها'};
...
async function render(id,background=false){const serial=(runs.get(id)||0)+1;runs.set(id,serial);const identity=window.bamcoAuth?.snapshot?.(),user=state.user?.id,current=()=>serial===runs.get(id)&&state.user?.id===user&&!!state.token&&(!identity||window.bamcoAuth.isCurrent(identity));const view=q('#'+id+'View'),owner=JSON.stringify(identity||[user,state.token]);if(view&&view.dataset.reportOwner!==owner){view.dataset.reportOwner=owner;delete view.dataset.sessionSnapshot;datasets.delete(id)}if(!background&&!datasets.has(id))panel(id,'<p role="status" class="workspace-loading">در حال دریافت اطلاعات…</p>');view?.setAttribute('aria-busy','true');try{
 ...
 if(id==='requestReport'){const rows=await refreshWorkflowRows();if(!current())return;report(id,['شناسه','درخواست‌دهنده','نوع درخواست','عنوان فعالیت','مرحله','وضعیت','زمان ایجاد','زمان بررسی','نظر مدیر'],rows.map(x=>[x.id,name(x.requested_by,x.requester_name_snapshot||'—'),requestWord(x),x.proposed_data?.title||state.tasks.find(t=>String(t.id)===String(x.task_id))?.title||'—',x.current_stage||1,word(x.request_status),dt(x.created_at),dt(x.reviewed_at),window.bamcoRequestManagerNote?.(x)||'—']),[['کل درخواست‌ها',rows.length],['در انتظار',rows.filter(x=>['pending','in_review'].includes(x.request_status)).length],['تعریف وظیفه جدید',rows.filter(x=>x.request_type==='create'&&['pending','in_review'].includes(x.request_status)).length]],i=>`<button class="ghost request-timeline-btn" data-request="${esc(rows[i].id)}">خط زمانی</button>`);return}
...
function bind(){
 ...
 const refreshVisibleReport=detail=>{const domain=String(detail?.domain||detail?.table||'').toLowerCase(),id=state.view;if(!state.token||!Object.hasOwn(titles,id))return;if(['workflow','tasks','profiles','notifications','access'].includes(domain)&&['performanceReport','requestReport','messageReport'].includes(id))void render(id,true);if((domain==='profiles'&&['activeSessions','loginActivity','loginReport'].includes(id))||['user_sessions','sessions'].includes(domain))refreshSessions()};
...
let sentRows=[],sentLoading=null,sentLoaded=false,sentDetailRun=0;
...
function sentBody({loading=false,error=''}={}){const filtered=sentFiltered();if(loading)return '<tr><td colspan="9" class="empty workspace-loading" role="status">در حال دریافت سابقه پیام‌ها…</td></tr>';if(error)return `<tr><td colspan="9" class="empty"><div class="workspace-error" role="alert"><b>سابقه پیام‌ها دریافت نشد.</b><p>${esc(error)}</p><button type="button" class="ghost" data-sent-retry>تلاش دوباره</button></div></td></tr>`;return filtered.length?filtered.map((r,i)=>`<tr data-sent-key="${esc(r.log_key)}"><td>${faNum(filtered.length-i)}</td><td>${esc(SOURCE_LABEL[r.source_type]||r.source_type||'—')}</td><td>${esc(r.sender_name||'سامانه')}</td><td>${esc(r.recipient_name||'—')}</td><td>${esc(r.subject||'—')}</td><td>${esc(SENT_CHANNEL[r.channel]||r.channel||'—')}</td><td><span class="sent-status ${esc(r.delivery_status||'')}">${esc(SENT_STATUS[r.delivery_status]||r.delivery_status||'—')}</span></td><td>${esc(sentDate(r.sent_at))}</td><td><button type="button" class="ghost" data-sent-detail="${esc(r.log_key)}">مشاهده</button></td></tr>`).join(''):'<tr><td colspan="9" class="empty">رکوردی مطابق فیلترها وجود ندارد.</td></tr>'}
function ensureSentShell(){const view=ensureSentView();if(!q('.sent-workspace',view))view.innerHTML='<div class="panel workspace-panel sent-workspace"><div class="panel-head"><div><h3>پیام‌های ارسال‌شده</h3><small>فقط گزارش وضعیت امور و یادآوری‌ها</small></div></div><div class="sent-command-row bamco-command-bar"><button type="button" class="ghost content-back">بازگشت به خانه</button><button type="button" class="ghost" data-sent-export>خروجی اکسل</button><button type="button" class="ghost" data-sent-refresh>تازه‌سازی</button><button type="button" class="ghost hidden" data-sent-access data-feature-access-control="local">مدیریت دسترسی</button></div><div class="table-wrap"><table class="workspace-table sent-table"><thead><tr><th>ردیف</th><th>نوع</th><th>فرستنده</th><th>گیرنده</th><th>موضوع</th><th>کانال</th><th>وضعیت</th><th>زمان ارسال</th><th>جزئیات</th></tr></thead><tbody>'+sentBody({loading:true})+'</tbody></table></div></div>';bindSentControls(view);syncSentAccess();return view}
function renderSent(options={}){const view=ensureSentShell(),body=q('.sent-table tbody',view);if(body)body.innerHTML=sentBody(options);syncSentAccess();return view}
function bindSentControls(view=ensureSentView()){if(view.dataset.sentControlsBound==='1')return;view.dataset.sentControlsBound='1';view.addEventListener('click',e=>{const target=e.target.closest?.('[data-sent-refresh],[data-sent-export],[data-sent-access],[data-sent-retry],[data-sent-detail],.content-back');if(!target||!view.contains(target))return;if(target.matches('[data-sent-refresh],[data-sent-retry]'))return void loadSent(true);if(target.matches('[data-sent-export]'))return void exportSent();if(target.matches('[data-sent-access]'))return void window.bamcoAccessEditor?.open?.({featureKey:sentFeature(),title:'مدیریت دسترسی پیام‌های ارسال‌شده'});if(target.matches('[data-sent-detail]')){e.stopPropagation();return void openSentDetail(target.dataset.sentDetail)}if(target.matches('.content-back'))return void window.bamcoShowHome?.()});view.addEventListener('dblclick',e=>{const row=e.target.closest?.('tr[data-sent-key]');if(row&&view.contains(row))void openSentDetail(row.dataset.sentKey)})}
async function exportSent(){if(!await ensureSentAccess('export'))return;const button=q('[data-sent-export]');if(button)button.disabled=true;try{const list=sentFiltered(),headers=['ردیف','نوع','فرستنده','گیرنده','موضوع','کانال','وضعیت','زمان ارسال'],data=[headers,...list.map((r,i)=>[list.length-i,SOURCE_LABEL[r.source_type]||r.source_type||'—',r.sender_name||'سامانه',r.recipient_name||'—',r.subject||'—',SENT_CHANNEL[r.channel]||r.channel||'—',SENT_STATUS[r.delivery_status]||r.delivery_status||'—',sentDate(r.sent_at)])];const X=await window.ensureBamcoXLSX(),ws=X.utils.aoa_to_sheet(data),wb=X.utils.book_new();ws['!views']=[{rightToLeft:true}];ws['!autofilter']={ref:X.utils.encode_range({s:{r:0,c:0},e:{r:Math.max(0,data.length-1),c:headers.length-1}})};ws['!cols']=headers.map((_,i)=>({wch:[8,18,22,22,32,16,16,22][i]}));X.utils.book_append_sheet(wb,ws,'پیام‌های ارسال‌شده');wb.Workbook={Views:[{RTL:true}]};X.writeFile(wb,'پیام‌های ارسال‌شده.xlsx',{compression:true})}catch(err){toast(err.message,true)}finally{if(button)button.disabled=!sentCan('export')}}
...
function sentDetailShell(r,content){return `<div class="modal-head"><div><h3>جزئیات پیام ارسال‌شده</h3><p>${esc(r.subject||'بدون موضوع')}</p></div><button type="button" class="clean-close" aria-label="بستن">×</button></div><div class="sent-detail-body"><div class="sent-detail-meta"><div><small>نوع</small><b>${esc(SOURCE_LABEL[r.source_type]||r.source_type||'—')}</b></div><div><small>فرستنده</small><b>${esc(r.sender_name||'سامانه')}</b></div><div><small>گیرنده</small><b>${esc(r.recipient_name||'—')}</b></div><div><small>کانال</small><b>${esc(SENT_CHANNEL[r.channel]||r.channel||'—')}</b></div><div><small>وضعیت</small><b>${esc(SENT_STATUS[r.delivery_status]||r.delivery_status||'—')}</b></div><div><small>زمان ارسال</small><b>${esc(sentDate(r.sent_at))}</b></div></div>${content}</div>`}
async function openSentDetail(key){if(!await ensureSentAccess())return;const r=sentRows.find(x=>String(x.log_key)===String(key));if(!r)return;const run=++sentDetailRun,dialog=ensureSentDialog();dialog.innerHTML=sentDetailShell(r,'<div class="sent-detail-loading" role="status">در حال دریافت متن واقعی پیام…</div>');q('.clean-close',dialog).onclick=()=>dialog.close();if(!dialog.open)dialog.showModal();let body='',rendered='';try{if(r.snapshot_id){const snapshot=(await select('message_snapshots',`select=*&id=eq.${encodeURIComponent(r.snapshot_id)}`))?.[0];if(snapshot&&window.BamcoMessageRender){let sticker='';try{sticker=await window.BamcoMessageRender.stickerUrl(snapshot)}catch{}rendered=window.BamcoMessageRender.html(snapshot,{stickerUrl:sticker,subject:snapshot.subject||r.subject,reportDate:snapshot.created_at||r.sent_at,senderName:r.sender_name||'سامانه'})}else if(snapshot)body=snapshot.final_text||snapshot.body||snapshot.body_template||''}else if(r.portal_message_id){const p=(await select('portal_messages',`select=*&id=eq.${encodeURIComponent(r.portal_message_id)}`))?.[0];if(p)body=p.body||p.message||p.content||''}else if(r.source_type==='system_chat'&&r.source_id){const m=(await select('chat_messages',`select=*&id=eq.${encodeURIComponent(r.source_id)}`))?.[0];if(m)body=m.body||''}}catch(error){body=error?.message||''}if(run!==sentDetailRun||!dialog.open)return;const fallback=body||'متن کامل این رویداد در رکورد یکپارچه ذخیره نشده است؛ موضوع و فراداده بالا معتبر است.';dialog.innerHTML=sentDetailShell(r,rendered?`<div class="sent-detail-render">${rendered}</div>`:`<div class="sent-detail-text">${esc(fallback)}</div>`);q('.clean-close',dialog).onclick=()=>dialog.close()}
async function loadSent(force=false){if(!await ensureSentAccess())return[];if(sentLoading&&!force)return sentLoading;const view=ensureSentShell();view.classList.add('bamco-view-settling');if(!sentLoaded)renderSent({loading:true});sentLoading=(async()=>{try{sentRows=await selectAll('sent_message_log','select=*&order=sent_at.desc');sentLoaded=true;if(typeof state==='undefined'||state.view==='sentMessages')renderSent()}catch(err){if(typeof state==='undefined'||state.view==='sentMessages')renderSent({error:err.message})}finally{view.classList.remove('bamco-view-settling');view.dispatchEvent(new CustomEvent('bamco-view-data-ready',{bubbles:true}));sentLoading=null}})();return sentLoading}
...
function systemPreview(data,messageId){const {payload,snap}=data,warning=(snap.warning_task_ids||[]).length,overdue=(snap.overdue_task_ids||[]).length,waiting=BamcoMessageRender.taskGroups(snap).waiting.length,when=typeof jalaliDateTime==='function'?jalaliDateTime(payload.created_at):String(payload.created_at||''),sender=payload.sender_name||'سامانه';return `<button type="button" class="system-message-open" data-system-message-open="${esc(messageId)}"><span class="system-message-open-head"><span class="system-message-open-title" style="text-align:right!important;text-align-last:right!important;direction:rtl!important;unicode-bidi:isolate!important">${esc(digits(BamcoMessageRender.normalizeReportDates(payload.subject||snap.subject||'پیام خودکار سامانه')))}</span><span class="system-message-open-date">${BamcoMessageRender.timestampHtml(payload.created_at)}</span></span><span class="system-message-open-meta">فرستنده: ${esc(sender)}</span><span class="system-message-open-chips">${warning?`<span class="system-message-chip warning">هشدار: ${digits(warning)}</span>`:''}${overdue?`<span class="system-message-chip overdue">دیرکرد: ${digits(overdue)}</span>`:''}${waiting?`<span class="system-message-chip waiting">منتظر پاسخ: ${digits(waiting)}</span>`:''}${!warning&&!overdue&&!waiting?'<span class="system-message-chip">پیام خودکار سامانه</span>':''}</span><span class="system-message-open-cta">مشاهده محتوای کامل <span aria-hidden="true">←</span></span></button>`}
...
function goTask(id){const task=(state.tasks||[]).find(t=>String(t.id)===String(id));if(!task)return toast('این وظیفه در کانبان فعلی پیدا نشد.',true);if(window.bamcoOpenTaskInKanban?.(task.id))return;q('#nav [data-view="kanban"]')?.click();window.bamcoFocusMessageTask?.(task.id)}
