...
function lateMobileCss(){if(typeof document==='undefined'||document.querySelector('#bamcoMobileFinalCss'))return;const link=document.createElement('link');link.id='bamcoMobileFinalCss';link.rel='stylesheet';link.href='assets/css/mobile-compat-20260911.css?v=toast-owner-20260914-25';document.body.append(link)}
...
if(typeof document!=='undefined'){const install=()=>{lateMobileCss();installTaskRouting()};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install()}
})();
;
...
/* source: assets/js/visual-polish.js */
...
/* source: assets/js/excel-io-fixes.js */
(()=>{
'use strict';
const q=s=>document.querySelector(s);
...
/* source: assets/js/runtime-fixes-20260911.js */
(()=>{
'use strict';
...
/* source: assets/js/dashboard-response-fixes-20260911.js */
(()=>{
'use strict';
...
/* source: assets/js/login-avatar-workload-fix-20260911.js */
...
/* source: assets/js/workload-segment-labels-20260911.js */
(()=>{
'use strict';
...
/* source: assets/js/admin-root-fixes-20260911.js */
(()=>{
'use strict';
...
function enforceActiveView(id){if(!id||typeof state==='undefined'||state.view!==id)return;const target=q('#'+id+'View');if(!target)return;qa('#appView .view,.workspace > .view').forEach(v=>v.classList.toggle('hidden',v!==target));target.classList.remove('hidden');qa('#nav button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===id))}
function installNavigationInvariant(){document.addEventListener('click',e=>{const b=e.target.closest?.('#nav button[data-view]');if(!b||b.disabled)return;const id=b.dataset.view;queueMicrotask(()=>enforceActiveView(id));requestAnimationFrame(()=>enforceActiveView(id));setTimeout(()=>enforceActiveView(id),120)},false)}
...
let sentRows=[],sentLoading=null,sentViewGuard=null;
...
function armSentViewGuard(view){sentViewGuard?.disconnect();sentViewGuard=new MutationObserver(()=>{if(typeof state==='undefined'||state.view!=='sentMessages'){sentViewGuard?.disconnect();sentViewGuard=null;return}if(view.classList.contains('hidden')){qa('#appView .view,.workspace > .view').forEach(v=>v.classList.toggle('hidden',v!==view));view.classList.remove('hidden','bamco-view-settling')}});sentViewGuard.observe(view,{attributes:true,attributeFilter:['class']})}
function activateSentView(){if(typeof isManager==='function'&&!isManager())return;const view=ensureSentView();window.bamcoLeaveHome?.();if(typeof state!=='undefined')state.view='sentMessages';document.body.classList.remove('welcome-active','card-home-active');document.body.classList.add('content-only');q('#welcomeView')?.classList.add('hidden');armSentViewGuard(view);qa('#appView .view,.workspace > .view').forEach(v=>v.classList.toggle('hidden',v!==view));view.classList.remove('hidden','bamco-view-settling');queueMicrotask(()=>{if(typeof state!=='undefined'&&state.view==='sentMessages')view.classList.remove('hidden')});qa('#nav button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view==='sentMessages'));const title=q('#viewTitle');if(title)title.textContent='پیام‌های ارسال‌شده';q('#addTaskBtn')?.classList.add('hidden');void loadSent()}
...
function installSentMessages(){installSentCss();ensureSentNav();ensureSentView();document.addEventListener('click',e=>{const nav=e.target.closest?.('#nav button[data-view="sentMessages"]');if(!nav||nav.disabled||typeof isManager==='function'&&!isManager())return;e.preventDefault();activateSentView()},true);window.bamcoSentMessages={open:activateSentView,refresh:()=>loadSent(true),rows:()=>sentRows.slice(),filtered:()=>sentFiltered().slice()}}
function boot(){installRequestGuard();installMessageTextPurge();installNavigationInvariant();installSentMessages();normalizeRequestExport()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();
;

/* source: assets/js/report-stability-fixes-20260911.js */
(()=>{
'use strict';
...
/* source: assets/js/messaging-history-root-20260911.js */
(()=>{'use strict';
...
/* source: assets/js/task-bulk-delete-20260912.js */
(()=>{
'use strict';
...
/* source: assets/js/task-toolbar-actions-20260912.js */
(()=>{
'use strict';
...
/* source: assets/js/final-production-fixes-20260911.js */
(()=>{
'use strict';
if(window.__bamcoFinalAvatarReportFixes20260912V5)return;
window.__bamcoFinalAvatarReportFixes20260912V5=true;
const q=(s,r=document)=>r?.querySelector?.(s)||null;
const qa=(s,r=document)=>[...(r?.querySelectorAll?.(s)||[])];
const REPORT_TITLES={performanceReport:'گزارش عملکرد',responseReport:'گزارش پاسخ‌ها'};
let reportRepairing=false,rawShowView=null,responseSettleTimer=0;
...
function refreshAvatar(){return window.refreshProfileAvatar?.()??false}
function boot(){installReportNavigation();window.bamcoFinalRuntime={refreshAvatar}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();
;
...
function init(){const view=q('#lettersView');if(!view)return;try{titles.letters='نامه‌ها'}catch{}view.innerHTML='<section id="lettersPanel" class="letters-panel"><div class="letter-toolbar"><button class="content-back ghost">بازگشت به خانه</button><button class="ghost" id="switchLetters">نامه ورودی / خروجی</button><button class="primary" id="addLetter">افزودن نامه</button><button class="ghost" id="refreshLetters">تازه‌سازی</button><button class="ghost" id="letterEditSelected" disabled>ویرایش</button><button class="ghost" id="letterDownloadSelected" disabled>دانلود</button><button class="danger" id="letterDeleteSelected" disabled>حذف</button><button class="ghost hidden" id="lettersAccess">مدیریت دسترسی</button><input type="search" id="lettersSearch" placeholder="جست‌وجوی نامه…"></div><div class="letters-heading"><h3 id="lettersModeTitle">نامه‌ها</h3></div><p id="lettersError" class="letter-error"></p><div id="lettersBody"></div></section>';
 ...
 let shown=false;new MutationObserver(()=>{const now=!view.classList.contains('hidden');if(now&&!shown){shown=true;if(direction){void load()}else{direction='outgoing';void load();choose()}}else if(!now&&shown){shown=false;clearSelection()}}).observe(view,{attributes:true,attributeFilter:['class']});access();syncActions()}
...
async function ready(){if(!supported())throw Error('این مرورگر از اعلان پس‌زمینه پشتیبانی نمی‌کند.');const base=new URL('./',document.baseURI),script=new URL('push-sw.js',document.baseURI),regs=await bounded(navigator.serviceWorker.getRegistrations());await Promise.all(regs.filter(r=>{const u=r.active?.scriptURL||r.waiting?.scriptURL||r.installing?.scriptURL||'';if(!u)return false;try{return new URL(u).origin===location.origin&&r.scope.startsWith(base.href)&&!new URL(u).pathname.endsWith('/push-sw.js')}catch{return false}}).map(r=>r.unregister().catch(()=>false)));registration=await bounded(navigator.serviceWorker.register(script.href,{scope:base.pathname,updateViaCache:'none'}));try{await bounded(registration.update(),8000)}catch{}if(!registration.active)await bounded(navigator.serviceWorker.ready);registration=await bounded(navigator.serviceWorker.getRegistration(base.href))||registration;return registration}
...
function mount(){const view=document.querySelector('#settingsView');const host=view?.querySelector('.manager-form')||view;if(!host||view.querySelector('#pushSettings'))return;
 ...
 button.onclick=async()=>{if(busy||!current())return;const disabling=button.dataset.enabled==='true';start(disabling?'در حال غیرفعال‌سازی اعلان…':'در حال فعال‌سازی اعلان…');try{
  ...
  else {const sub=await subscription(reg,state.token,{renew:true});if(!current())return;await call('subscribe',{subscription:sub.toJSON()},state.token);if(!current())return;preference(false);button.dataset.enabled='true';button.textContent='غیرفعال‌سازی اعلان';status.textContent='اعلان این دستگاه فعال است؛ حتی هنگام بسته‌بودن سامانه.'}
