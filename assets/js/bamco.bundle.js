...
(() => {
  ...
  async function saveProject(event) { event.preventDefault(); const form = event.target, button = q('[type=submit]', form); setBusy(button, true); try { const payload = { project_code: form.elements.project_code.value.trim(), title: form.elements.title.value.trim(), description: form.elements.description.value.trim() || null, owner_id: form.elements.owner_id.value || state.user.id, manager_id: form.elements.manager_id.value || null, planned_start: form.elements.planned_start.value || null, planned_end: form.elements.planned_end.value || null, status: form.elements.status.value, priority: form.elements.priority.value, created_by: state.user.id }; const created=await insert('projects', payload); q('#projectDialog',root()).close(); model.selected=Array.isArray(created)?created[0]?.id||null:created?.id||null; notify('پروژه ایجاد شد.'); await load(); } catch (error) { notify(error.message, true); } finally { setBusy(button, false); } }
...
(() => {
  ...
  async function saveInvoice(event) { event.preventDefault(); const form = event.target, button = q('[type=submit]', form); setBusy(button, true); try { const created=await insert('invoices', { invoice_number: form.elements.invoice_number.value.trim(), title: form.elements.title.value.trim(), account_party: form.elements.account_party.value.trim(), company_name: form.elements.company_name.value.trim() || null, expense_type: form.elements.expense_type.value.trim() || null, currency: form.elements.currency.value, total_amount: Number(form.elements.total_amount.value || 0), due_date: form.elements.due_date.value || null, project_id: form.elements.project_id.value ? Number(form.elements.project_id.value) : null, part_id: form.elements.part_id.value ? Number(form.elements.part_id.value) : null, description: form.elements.description.value.trim() || null, created_by: state.user.id }); q('#invoiceDialog',root()).close(); model.selected=Array.isArray(created)?created[0]?.id||null:created?.id||null; notify('صورتحساب ثبت شد.'); await load(); } catch (error) { notify(error.message, true); } finally { setBusy(button, false); } }
