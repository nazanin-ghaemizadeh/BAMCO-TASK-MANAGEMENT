...
(() => {
  ...
  const model = { projects: [], items: [], dependencies: [], selected: null, search: '', detailView: 'gantt', projectEditor: null, itemEditor: null, dependencyEditor: null };
  let loadVersion = 0;
  ...
  function workBreakdownMarkup(projectRow, rows) {
    ...
    const card = (row, level = 0) => `<article class="wbs-node wbs-level-${Math.min(level, 4)}"><div class="wbs-node-line"><span class="wbs-type">${esc(itemTypeText(row.item_type))}</span><div><b>${esc(row.title)}</b><small>${row.item_type === 'milestone' ? `تاریخ نقطه عطف: ${date(row.planned_start)}` : `${date(row.planned_start)} تا ${date(row.planned_end)} · ${fa(Math.round(row.progress || 0))}٪`}</small></div><div class="wbs-node-actions"><button type="button" class="ghost" data-project-item-edit="${row.id}">ویرایش</button><button type="button" class="danger" data-project-item-delete="${row.id}">حذف</button></div></div>${(byParent.get(String(row.id)) || []).map(child => card(child, level + 1)).join('')}</article>`;
  ...
  function ganttMarkup(rows) {
    ...
    return `<div class="gantt-pro-wrap"><div class="gantt-pro"><aside class="gantt-pro-labels"><div class="gantt-pro-label-head">فعالیت‌های پروژه</div>${timelineRows.map(({ row, level }) => `<div class="gantt-pro-label-row ${row.item_type === 'phase' ? 'phase-row' : ''}" style="--level:${level}"><b>${esc(row.title)}</b><small>${esc(itemTypeText(row.item_type))}${row.item_type !== 'milestone' ? ` · ${fa(Math.round(row.progress || 0))}٪` : ''}</small></div>`).join('')}</aside><div class="gantt-pro-timeline" style="width:${width}px"><div class="gantt-pro-months">${months.map(month => `<span style="width:${month.count * cell}px">${esc(month.key)}</span>`).join('')}</div><div class="gantt-pro-days">${days.map(value => `<span>${esc(date(value).split('/').at(-1))}</span>`).join('')}</div><svg class="gantt-dependencies" viewBox="0 0 ${width} ${headerH + timelineRows.length * rowH}" width="${width}" height="${headerH + timelineRows.length * rowH}" aria-label="روابط پیش‌نیازی و پس‌نیازی"><defs><marker id="ganttArrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L7,3 z"></path></marker></defs>${relationSvg}</svg>${timelineRows.map(({ row }) => { const from = Math.floor((stamp(row.planned_start || row.planned_end) - start) / day), to = Math.floor((stamp(row.planned_end || row.planned_start) - start) / day), duration = Math.max(1, to - from + 1); return `<div class="gantt-pro-track ${row.item_type === 'phase' ? 'phase-row' : ''}">${row.item_type === 'milestone' ? `<i class="gantt-pro-milestone" style="left:${from * cell + cell / 2 - 8}px" title="${esc(row.title)}"></i>` : `<i class="gantt-pro-bar" style="left:${from * cell + 3}px;width:${Math.max(10, duration * cell - 6)}px"><span>${fa(Math.round(row.progress || 0))}٪</span></i>`}</div>`; }).join('')}</div></div><div class="gantt-legend"><span><i class="gantt-legend-phase"></i>فاز</span><span><i class="gantt-legend-activity"></i>فعالیت</span><span><i class="gantt-legend-milestone"></i>نقطه عطف</span><span><i class="gantt-legend-dependency"></i>رابطه پیش‌نیازی/پس‌نیازی</span></div></div>`;
  }
  ...
  function bind() {
    ...
    host.addEventListener('click', event => { const action = event.target.closest('[data-project-action]')?.dataset.projectAction; if (action === 'new') return showProjectDialog(); if (action === 'edit') return showProjectDialog(model.selected); if (action === 'delete') return void removeProject(); if (action === 'refresh') return void load(); if (action === 'back') { model.selected = null; model.detailView = 'gantt'; return render(); } if (action === 'item') return showItemDialog(); if (action === 'dependency') return showDependencyDialog(); if (event.target.closest('[data-project-close]')) { model.projectEditor = null; model.itemEditor = null; model.dependencyEditor = null; event.target.closest('dialog')?.close(); return; } const dateButton = event.target.closest('[data-project-date]'); if (dateButton) return openProjectDate(dateButton); const select = event.target.closest('[data-project-select]'); if (select) { model.selected = select.dataset.projectSelect; model.detailView = 'gantt'; return render(); } const view = event.target.closest('[data-project-view]'); if (view) { model.detailView = view.dataset.projectView; return render(); } const editItem = event.target.closest('[data-project-item-edit]'); if (editItem) return showItemDialog(editItem.dataset.projectItemEdit); const deleteItem = event.target.closest('[data-project-item-delete]'); if (deleteItem) return void removeItem(deleteItem.dataset.projectItemDelete); const editDependency = event.target.closest('[data-project-dependency-edit]'); if (editDependency) return showDependencyDialog(editDependency.dataset.projectDependencyEdit); const deleteDependency = event.target.closest('[data-project-dependency-delete]'); if (deleteDependency) return void removeDependency(deleteDependency.dataset.projectDependencyDelete); });
