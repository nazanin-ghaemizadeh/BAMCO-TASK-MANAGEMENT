...
(() => {
  ...
  function workBreakdownMarkup(projectRow, rows) {
    ...
    const visit = (row, depth = 1) => { if (seen.has(String(row.id))) return; seen.add(String(row.id)); ordered.push({ row, depth }); (byParent.get(String(row.id)) || []).forEach(child => visit(child, depth + 1)); };
    (byParent.get('__project__') || []).forEach(row => visit(row)); rows.forEach(row => visit(row));
    ...
    const itemCards = ordered.map(({ row, depth }) => { const meta = phaseMeta(row, rows), parent = row.parent_item_id && known.has(String(row.parent_item_id)) ? row.parent_item_id : 'project-root', progressText = row.item_type === 'milestone' ? date(row.planned_start) : `${fa(Math.round(calculatedProgress(row)))}٪ پیشرفت`; return `<article class="project-wbs-card ${row.item_type}" data-wbs-node="${row.id}" data-wbs-parent="${parent}" data-project-item-open="${row.id}" style="--wbs-depth:${depth};--wbs-indent:${depth * 38}px;--phase-color:${meta.color}" title="برای ویرایش دوبار کلیک کنید"><span>${esc(itemLabel(row))}</span><b>${esc(row.title)}</b><small>${progressText}</small></article>`; }).join('');
    return `<div class="wbs-direct"><div class="project-wbs-canvas"><svg class="project-wbs-connectors" aria-label="اتصالات ساختار شکست"></svg><div class="project-wbs-tree">${rootCard}${itemCards || '<div class="empty">برای شروع، یک فاز، فعالیت یا نقطه عطف اضافه کنید.</div>'}</div></div></div>`;
  }
  function ganttMarkup(rows) {
    ...
    const count = Math.max(1, Math.min(180, Math.floor((end - start) / day) + 1)), viewport = Math.max(860, Number(window.innerWidth || 1280)), cell = Math.max(28, Math.ceil((viewport - 330) / count)), days = Array.from({ length: count }, (_, index) => new Date(start + index * day).toISOString().slice(0, 10)), width = count * cell;
  ...
  function drawWbsConnectors() {
    ...
    const links = [...nodes.values()].filter(node => node.dataset.wbsParent && nodes.has(String(node.dataset.wbsParent))).map(node => {
      ...
      const x1 = Math.round(a.right - box.left - 18), y1 = Math.round(a.bottom - box.top), x2 = Math.round(b.right - box.left - 18), y2 = Math.round(b.top - box.top), bend = Math.round((y1 + y2) / 2);
