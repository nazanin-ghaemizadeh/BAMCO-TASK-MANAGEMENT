...
(() => {
  ...
  const phasePalette = ['#176b4d', '#2563a8', '#b96b20', '#8653a1', '#167c83', '#b6525c'];
  ...
  function workBreakdownMarkup(projectRow, rows) {
    if (!rows.length) return `<div class="wbs-direct"><div class="wbs-root"><b>${esc(projectRow.title)}</b><small>${fa(Math.round(projectProgress(projectRow)))}٪ پیشرفت</small></div><div class="empty">برای شروع، یک فاز، فعالیت یا نقطه عطف اضافه کنید.</div></div>`;
    ...
    const node = (row, depth) => { const meta = phaseMeta(row, rows); return `<div class="wbs-tree-node ${row.item_type} wbs-level-${depth}" data-project-item-open="${row.id}" style="--phase-color:${meta.color}" title="برای ویرایش دوبار کلیک کنید"><span>${esc(itemLabel(row))}</span><b>${esc(row.title)}</b><small>${row.item_type === 'milestone' ? date(row.planned_start) : `${fa(Math.round(calculatedProgress(row)))}٪ پیشرفت`}</small></div>`; };
    const branch = (row, depth = 0) => { const children = byParent.get(String(row.id)) || []; return `<div class="wbs-branch ${children.length ? 'has-children' : ''}">${node(row, depth)}${children.length ? `<div class="wbs-subtree">${children.map(child => branch(child, depth + 1)).join('')}</div>` : ''}</div>`; };
    const top = byParent.get('__top__') || [], phases = top.filter(row => row.item_type === 'phase'), other = top.filter(row => row.item_type !== 'phase');
    return `<div class="wbs-direct"><div class="wbs-root"><b>${esc(projectRow.title)}</b><small>${fa(Math.round(projectProgress(projectRow)))}٪ پیشرفت</small></div><div class="wbs-phase-grid">${phases.map(phase => `<section class="wbs-phase-branch" style="--phase-color:${phaseMeta(phase, rows).color}">${branch(phase)}</section>`).join('')}${other.length ? `<section class="wbs-phase-branch wbs-unphased">${other.map(row => branch(row)).join('')}</section>` : ''}</div></div>`;
  }
  function ganttMarkup(rows) {
    ...
    const relationSvg = projectDependencies(rows).filter(dep => pos.has(String(dep.predecessor_item_id)) && pos.has(String(dep.successor_item_id))).map(dep => { const from = item(dep.predecessor_item_id), to = item(dep.successor_item_id), kind = dep.dependency_type || 'FS', map = { FS: ['end','start'], SS: ['start','start'], FF: ['end','end'], SF: ['start','end'] }[kind] || ['end','start'], x1 = x(from, map[0]), x2 = x(to, map[1]), y1 = headerH + pos.get(String(from.id)) * rowH + rowH / 2, y2 = headerH + pos.get(String(to.id)) * rowH + rowH / 2, bend = x2 > x1 ? x1 + Math.max(28, (x2 - x1) / 2) : x1 - Math.max(28, (x1 - x2) / 2); return `<path d="M ${x1} ${y1} C ${bend} ${y1}, ${bend} ${y2}, ${x2} ${y2}" class="gantt-dependency-line" marker-end="url(#ganttArrow)"><title>${kind} · ${esc(from.title)} ← ${esc(to.title)}</title></path>`; }).join('');
    ...
    return `<div class="gantt-pro-wrap gantt-direct"><div class="gantt-pro"><aside class="gantt-pro-labels">${timelineRows.map(({ row, level }) => { const meta = phaseMeta(row, rows); return `<div class="gantt-pro-label-row ${row.item_type === 'phase' ? 'phase-row' : ''} ${level > 1 ? 'subactivity-row' : ''}" data-project-item-open="${row.id}" title="برای ویرایش دوبار کلیک کنید" style="--level:${level};--phase-color:${meta.color}"><b>${esc(row.title)}</b><small>${esc(itemLabel(row))}${row.item_type !== 'milestone' ? ` · ${fa(Math.round(calculatedProgress(row)))}٪` : ''}</small></div>`; }).join('')}</aside><div class="gantt-pro-timeline" style="width:${width}px"><div class="gantt-pro-months">${months.map(month => `<span style="width:${month.count * cell}px">${esc(month.key)}</span>`).join('')}</div><div class="gantt-pro-days">${days.map(value => `<span>${esc(date(value).split('/').at(-1))}</span>`).join('')}</div><svg class="gantt-dependencies" viewBox="0 0 ${width} ${headerH + timelineRows.length * rowH}" width="${width}" height="${headerH + timelineRows.length * rowH}" aria-label="روابط پیش‌نیازی و پس‌نیازی"><defs><marker id="ganttArrow" markerWidth="12" markerHeight="12" refX="10" refY="5" orient="auto"><path d="M0,0 L0,10 L10,5 z"></path></marker></defs>${relationSvg}</svg>${timelineRows.map(({ row, level }) => { const from = Math.floor((stamp(row.planned_start || row.planned_end) - start) / day), to = Math.floor((stamp(row.planned_end || row.planned_start) - start) / day), duration = Math.max(1, to - from + 1), meta = phaseMeta(row, rows); return `<div class="gantt-pro-track ${row.item_type === 'phase' ? 'phase-row' : ''} ${level > 1 ? 'subactivity-row' : ''}" data-project-item-open="${row.id}" title="برای ویرایش دوبار کلیک کنید" style="--phase-color:${meta.color}">${row.item_type === 'milestone' ? `<i class="gantt-pro-milestone" style="right:${from * cell + cell / 2 - 8}px" title="${esc(row.title)}"></i>` : `<i class="gantt-pro-bar" style="right:${from * cell + 3}px;width:${Math.max(10, duration * cell - 6)}px"><span>${fa(Math.round(calculatedProgress(row)))}٪</span></i>`}</div>`; }).join('')}</div></div></div>`;
  }
