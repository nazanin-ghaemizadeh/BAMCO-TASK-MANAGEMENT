Warning: truncated output (original token count: 245177)
Total output lines: 8974

/* Generated by scripts/build-static-bundles.mjs. Do not edit. */
/* sha256: 13397b02a91ffe31 */
...
(() => {
  ...
  const model = { projects: [], items: [], dependencies: [], selected: null, search: '', loaded: false, detailView: 'wbs', ganttOpen: false, projectEditor: null, itemEditor: null, dependencyEditor: null };
  let loadVersion = 0;
  ...
  function render() {
    ...
    const commandBar = current ? `<div class="project-top-command-row"><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><button type="button" class="ghost" data-project-action="back">بازگشت به پروژه‌ها</button><button type="button" class="ghost" data-project-action="edit">ویرایش پروژه</button><button type="button" class="danger" data-project-action="delete">حذف پروژه</button><button type="button" class="primary" data-project-action="item">افزودن فعالیت</button><button type="button" class="ghost" data-project-action="dependency">مدیریت روابط</button><button type="button" class="ghost" data-project-view="gantt">نمای گانت</button><button type="button" class="ghost active" data-project-view="wbs">نمای ساختار شکست</button><input id="projectSearch" type="search" value="${esc(model.search)}" placeholder="جست‌وجوی پروژه…"><button type="button" class="ghost" data-project-action="refresh">تازه‌سازی</button></div>` : `<div class="project-top-command-row"><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><input id="projectSearch" type="search" value="${esc(model.search)}" placeholder="جست‌وجوی پروژه…"><button type="button" class="primary" data-project-action="new">افزودن پروژه</button><button type="button" class="ghost" data-project-action="refresh">تازه‌سازی</button></div>`;
    ...
    host.innerHTML = `<div class="feature-toolbar enterprise-toolbar"><div><h3>مدیریت پروژه‌ها</h3><p>زمان‌بندی، ساختار کار، وابستگی‌ها و پیشرفت پروژه در یک نمای یکپارچه.</p></div></div>${commandBar}<div class="enterprise-grid project-grid ${current ? 'detail-open' : ''}"><section class="panel"><div class="panel-head"><h3>${scopeTitle()}</h3><span class="enterprise-count">${fa(visible.length)} پروژه</span></div><div class="enterprise-card-list">${listContent}</div></section><section class="panel project-detail">${current ? detailMarkup(current) : '<div class="enterprise-empty"><b>پروژه‌ای انتخاب نشده است.</b><span>برای شروع، پروژه‌ای اضافه کنید یا یکی از کارت‌ها را انتخاب کنید.</span></div>'}</section></div>${projectDialogMarkup()}${current ? itemDialogMarkup(current) + dependencyDialogMarkup(current) + ganttDialogMarkup(current) : ''}`;
    bind();
  }
  ...
  function workBreakdownMarkup(projectRow, rows) {
    ...
    const card = (row, level = 0) => `<article class="wbs-node wbs-level-${Math.min(level, 4)}"><div class="wbs-node-line" data-project-item-open="${row.id}" title="برای ویرایش دوبار کلیک کنید"><span class="wbs-type">${esc(itemTypeText(row.item_type))}</span><div><b>${esc(row.title)}</b><small>${row.item_type === 'milestone' ? `تاریخ نقطه عطف: ${date(row.planned_start)}` : `${date(row.planned_start)} تا ${date(row.planned_end)} · ${fa(Math.round(calculatedProgress(row)))}٪`}</small></div><div class="wbs-node-actions"><button type="button" class="ghost" data-project-item-edit="${row.id}">ویرایش</button><button type="button" class="danger" data-project-item-delete="${row.id}">حذف</button></div></div>${(byParent.get(String(row.id)) || []).map(child => card(child, level + 1)).join('')}</article>`;
