...
(() => {
  ...
  function syncManageControl() {
    ...
    // Card navigation intentionally hides the old workspace header while a
    // feature is open. Keep the same control usable there instead of making
    // access management disappear merely because the layout changed.
    const contentOnly = document.body?.classList.contains('content-only');
    const host = contentOnly
      ? (document.querySelector('#appView') || document.body)
      : (document.querySelector('.workspace > header .header-actions') || document.querySelector('.header-actions'));
    if (!host) return;
    let button = document.getElementById('featureAccessControl');
    if (!button) {
      button = document.createElement('button');
      button.id = 'featureAccessControl';
      button.type = 'button';
      button.className = 'ghost feature-access-control hidden';
      button.textContent = 'مدیریت دسترسی';
      button.addEventListener('click', () => {
        const featureKey = button.dataset.featureKey;
        if (!featureKey || !canManageFeature(featureKey)) return denied(featureKey || 'settings', 'manage_access');
        window.bamcoAccessEditor?.open?.({ featureKey, title: `مدیریت دسترسی ${catalog.featureTitle(featureKey)}` });
      });
      host.append(button);
    } else if (button.parentElement !== host) host.append(button);
    const featureKey = catalog.featureForRoute(state().view);
  ...
  function applyNavigation() {
    ...
    document.querySelectorAll('[data-feature-key][data-feature-action]').forEach(node => {
      ...
      // A feature grant only removes the authorization barrier.  It must not
      // re-enable a control whose own renderer has disabled it for a local
      // prerequisite (no selected rows, invalid form, in-flight save, etc.).
