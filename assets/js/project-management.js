...
(() => {
  ...
  const { q, esc, fa, date, dateTime, progress, statusText, fetchRows, insert, insertMinimal, update, rpc, removeRows, setBusy, notify } = E;
  ...
  const legacyStatusKey = value => ({ draft: 'registered', planned: 'registered', in_progress: 'doing', waiting: 'waiting', completed: 'done' }[value] || value || 'registered');
  const canonicalStatus = value => window.bamcoOptions?.label?.('status', legacyStatusKey(value)) || value || 'ثبت شده';
  const statusOptions = value => {
    const selectedValue = canonicalStatus(value), catalog = window.bamcoOptions?.rows?.('status') || [];
    const rows = catalog.filter(row => row.active || row.label === selectedValue);
    const labels = rows.map(row => row.label); if (selectedValue && !labels.includes(selectedValue)) labels.push(selectedValue);
    return labels.map(label => `<option value="${esc(label)}" ${label === selectedValue ? 'selected' : ''}>${esc(label)}</option>`).join('');
  };
  const profileLabel = person => person?.display_name || person?.full_name || person?.excel_name || person?.email || '—';
  const profileDirectory = () => {
    const canonical = window.BamcoProfiles;
    if (typeof canonical?.list === 'function') return canonical.list();
    if (typeof canonical?.all === 'function') return canonical.all();
    return state.profiles || [];
  };
  const projectOwnerChoices = editing => {
    const currentId = state.user?.id;
    const organization = window.bamcoOrganizationAccess;
    const descendants = typeof organization?.descendantUserIds === 'function'
      ? organization.descendantUserIds()
      : state.organizationScope?.descendantUserIds || [];
    const directReports = typeof organization?.directReportUserIds === 'function'
      ? organization.directReportUserIds()
      : state.organizationScope?.directReportUserIds || [];
    const allowedIds = [...new Set([currentId, ...directReports, ...descendants].filter(Boolean).map(String))];
    const people = new Map(profileDirectory().filter(person => person?.id).map(person => [String(person.id), person]));
    if (state.profile?.id) people.set(String(state.profile.id), { ...people.get(String(state.profile.id)), ...state.profile });
    const choices = allowedIds.map(id => people.get(id) || (id === String(currentId) ? { id: currentId, display_name: profileLabel(state.profile) } : null)).filter(person => person && person.active !== false);
    const selectedId = editing?.owner_id && String(editing.owner_id);
    if (selectedId && !choices.some(person => String(person.id) === selectedId)) {
      const existing = people.get(selectedId);
      if (existing) choices.push({ ...existing, outsideCurrentScope: true });
    }
    return choices;
  };
  const projectOwnerOptions = editing => {
    const selectedId = String(editing?.owner_id || state.user?.id || '');
    return projectOwnerChoices(editing).map(person => `<option value="${esc(person.id)}" ${String(person.id) === selectedId ? 'selected' : ''} ${person.outsideCurrentScope ? 'disabled' : ''}>${esc(profileLabel(person))}${person.outsideCurrentScope ? ' (خارج از زیرمجموعهٔ فعلی)' : ''}</option>`).join('');
  };
  ...
  function render() {
    ...
    const commandBar = current ? `<div class="project-top-command-row bamco-command-bar"><span data-feature-access-suppressed="true" data-home-return-suppressed="true" hidden></span><button type="button" class="ghost" data-project-action="back">بازگشت به پروژه‌ها</button><button type="button" class="ghost" data-project-action="edit">ویرایش پروژه</button><button type="button" class="danger" data-project-action="delete">حذف پروژه</button><button type="button" class="primary" data-project-action="item">افزودن فعالیت</button><button type="button" class="ghost" data-project-action="dependency">مدیریت روابط</button><button type="button" class="ghost ${model.detailView === 'gantt' ? 'active' : ''}" data-project-view="gantt">نمای گانت</button><button type="button" class="ghost ${model.detailView === 'wbs' ? 'active' : ''}" data-project-view="wbs">نمای ساختار شکست</button>${projectSearch}</div>` : `<div class="project-top-command-row bamco-command-bar"><button type="button" class="ghost" data-home-action>بازگشت به خانه</button><button type="button" class="primary" data-project-action="new">افزودن پروژه</button>${accessControl}<button type="button" class="ghost" data-project-action="refresh">تازه‌سازی</button>${projectSearch}</div>`;
  ...
  function projectDialogMarkup() {
    ...
    const ownerLocked = !!editing && items(editing.id).length > 0;
    return `<dialog id="projectDialog" class="modal enterprise-modal project-dialog"><form id="projectForm" method="dialog"><input type="hidden" name="project_id" value="${editing?.id || ''}"><div class="modal-head"><div><h3>${editing ? 'ویرایش پروژه' : 'افزودن پروژه'}</h3></div><button type="button" data-project-close aria-label="بستن">×</button></div><div class="form-grid"><label>عنوان پروژه<input name="title" value="${esc(editing?.title || '')}" required></label><label ${ownerLocked ? 'title="پس از ثبت اولین فعالیت، متولی پروژه قابل تغییر نیست."' : ''}>متولی<select name="project_owner_id" required ${ownerLocked ? 'disabled' : ''}>${projectOwnerOptions(editing)}</select>${ownerLocked ? '<small>پس از ثبت اولین فعالیت قابل تغییر نیست.</small>' : ''}</label><label>وضعیت<select name="status">${statusOptions(editing?.status)}</select></label><label>اولویت<select name="priority">${priorities.map(([value,label]) => `<option value="${value}" ${editing?.priority === value || (!editing && value === 'medium') ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label class="span-2">توضیحات<textarea name="description" rows="3">${esc(editing?.description || '')}</textarea></label>${dateField('planned_start', 'شروع برنامه‌ای', editing?.planned_start)}${dateField('planned_end', 'پایان برنامه‌ای', editing?.planned_end)}</div><div class="modal-actions"><button type="button" class="ghost" data-project-close>انصراف</button><button type="submit" class="primary">${editing ? 'ذخیره تغییرات' : 'ثبت پروژه'}</button></div></form></dialog>`;
  }
  ...
  async function saveProject(event) {
    event.preventDefault(); const form = event.target, button = q('[type=submit]', form), id = form.elements.project_id.value, editing = project(id), ownerId = form.elements.project_owner_id.value;
    if (!projectOwnerChoices(editing).some(person => String(person.id) === String(ownerId))) return notify('متولی انتخاب‌شده در دامنهٔ سازمانی شما نیست.', true);
    const payload = { title: form.elements.title.value.trim(), description: form.elements.description.value.trim() || null, owner_id: ownerId, manager_id: ownerId, planned_start: form.elements.planned_start.value || null, planned_end: form.elements.planned_end.value || null, status: form.elements.status.value, priority: form.elements.priority.value, updated_by: state.user.id };
    ...
    setBusy(button, true); try { if (id) { await update('projects', `id=eq.${encodeURIComponent(id)}`, payload); model.selected = id; notify('پروژه ویرایش شد.'); } else { const code = generatedProjectCode(); await (insertMinimal || insert)('projects', { ...payload, project_code: code, created_by: state.user.id }); model.projectEditor = null; await load(); model.selected = model.projects.find(row => row.project_code === code)?.id || null; render(); notify('پروژه ایجاد شد.'); return; } model.projectEditor = null; q('#projectDialog', root())?.close(); await load(); render(); } catch (error) { notify(error.message, true); } finally { setBusy(button, false); }
  }
  async function saveItem(event) {
    ...
    const existing = item(id), projectRow = project(form.elements.project_id.value), projectId = Number(form.elements.project_id.value), ownerId = projectRow?.owner_id || state.user.id;
    const activityPayload = { title: form.elements.title.value.trim(), description: existing?.description || null, owner_id: ownerId, parent_item_id: parentId, planned_start: start || null, planned_end: end || null, status: form.elements.status.value, priority: form.elements.priority.value, progress: Number(form.elements.progress.value || 0) };
    const directPayload = { project_id: projectId, item_type: type, ...activityPayload, progress: type === 'phase' ? Number(existing?.progress || 0) : activityPayload.progress, weight: 1, created_by: state.user.id };
    setBusy(button, true); try {
      if (type === 'activity') {
        const result = await rpc('save_project_activity', { p_item_id: id ? Number(id) : null, p_project_id: projectId, p_payload: activityPayload });
        if (result?.applied_directly === false) notify(id && existing?.task_id != null ? 'درخواست ویرایش فعالیت در پروژه برای تأیید بالادست ارسال شد.' : 'درخواست تعریف فعالیت در پروژه برای تأیید بالادست ارسال شد.');
        else notify(id ? 'فعالیت ویرایش شد.' : 'فعالیت اضافه شد.');
      } else if (id) {
        delete directPayload.created_by; await update('project_items', `id=eq.${encodeURIComponent(id)}`, directPayload); notify('فعالیت ویرایش شد.');
      } else {
        await insert('project_items', directPayload); notify('فعالیت اضافه شد.');
      }
      model.itemEditor = null; q('#projectItemDialog', root())?.close(); await load();
  ...
  async function removeProject() { const current = selected(); if (!current) return; const ok = window.bamcoConfirm ? await window.bamcoConfirm(`درخواست حذف پروژه «${current.title}» و همه فعالیت‌های آن برای تأیید بالادست ارسال شود؟`) : window.confirm(`درخواست حذف پروژه «${current.title}» ارسال شود؟`); if (!ok) return; try { await rpc('request_project_deletion', { p_project_id: Number(current.id) }); notify('درخواست حذف پروژه برای تأیید بالادست ارسال شد. پروژه تا زمان تأیید باقی می‌ماند.'); await load(); } catch (error) { notify(error.message, true); } }
  async function removeItem(id) { const row = item(id); if (!row) return; const ok = window.bamcoConfirm ? await window.bamcoConfirm(`فعالیت «${row.title}» و زیرمجموعه‌ها و روابط وابسته حذف شوند؟`) : window.confirm(`فعالیت «${row.title}» حذف شود؟`); if (!ok) return; try { if (row.item_type === 'activity') { const result = await rpc('delete_project_activity', { p_item_id: Number(id) }); notify(result?.applied_directly === false ? 'درخواست حذف فعالیت از پروژه برای تأیید بالادست ارسال شد.' : 'فعالیت حذف شد.'); } else { await removeRows('project_items', `id=eq.${encodeURIComponent(id)}`); notify('فعالیت حذف شد.'); } model.itemEditor = null; q('#projectItemDialog', root())?.close(); await load(); } catch (error) { notify(error.message, true); } }
  ...
  function boot() { if (!root()) return; render(); document.addEventListener('bamco:task-options', () => { if (root()) render(); }); window.BamcoNavigation?.registerView?.('projects', { activate: load }); }
