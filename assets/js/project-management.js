...
(() => {
  ...
  function ganttMarkup(rows) {
    ...
    const stamp = value => value ? Date.parse(`${value}T00:00:00Z`) : null, day = 86400000, rowH = 58, headerH = 72;
    ...
    const relationSvg = projectDependencies(rows).filter(dep => pos.has(String(dep.predecessor_item_id)) && pos.has(String(dep.successor_item_id))).map(dep => { const from = item(dep.predecessor_item_id), to = item(dep.successor_item_id), kind = dep.dependency_type || 'FS', map = { FS: ['end','start'], SS: ['start','start'], FF: ['end','end'], SF: ['start','end'] }[kind] || ['end','start'], x1 = x(from, map[0]), x2 = x(to, map[1]), y1 = headerH + pos.get(String(from.id)) * rowH + rowH / 2, y2 = headerH + pos.get(String(to.id)) * rowH + rowH / 2, lane = Math.round((x1 + x2) / 2), path = Math.abs(y1 - y2) < 2 ? `M ${x1} ${y1} H ${x2}` : `M ${x1} ${y1} H ${lane} V ${y2} H ${x2}`; return `<path d="${path}" class="gantt-dependency-line" marker-end="url(#ganttArrow)"><title>${kind} · ${esc(from.title)} ← ${esc(to.title)}</title></path>`; }).join('');
    ...
    return `<div class="gantt-pro-wrap gantt-direct"><div class="gantt-pro"><aside class="gantt-pro-labels"><div class="gantt-pro-label-spacer" aria-hidden="true"></div>${labelRows}</aside><div class="gantt-pro-timeline-scroll"><div class="gantt-pro-timeline" style="width:${width}px;--gantt-cell-width:${cell}px"><div class="gantt-pro-months">${months.map(month => `<span style="width:${month.count * cell}px">${esc(month.key)}</span>`).join('')}</div><div class="gantt-pro-days">${days.map(value => `<span style="width:${cell}px">${esc(date(value).split('/').at(-1))}</span>`).join('')}</div><svg class="gantt-dependencies" viewBox="0 0 ${width} ${headerH + timelineRows.length * rowH}" width="${width}" height="${headerH + timelineRows.length * rowH}" aria-label="روابط پیش‌نیازی و پس‌نیازی"><defs><marker id="ganttArrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L0,7 L6,3.5 z"></path></marker></defs>${relationSvg}</svg>${trackRows}</div></div></div></div>`;
  }
