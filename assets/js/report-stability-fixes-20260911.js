...
async function renderPerformance(force=false){
 ...
 const tasks=state.tasks||[],ids=[...new Set([...tasks.map(t=>t.owner_id),...tasks.map(t=>t.created_by),...requests.map(r=>r.requested_by)])].filter(Boolean),defaultRange=!!range&&from===range.from&&to===range.to;
