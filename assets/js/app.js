...
function requestTypeLabel(row){
  ...
  if(row?.proposed_data?.request_context==='project_delete')return'حذف پروژه';
...
function renderRequests(){
  ...
  $('#approvalBody').innerHTML=rows.length?rows.map((r,index)=>{
    ...
    if(terminal)action='—';
    else if(r.request_status==='needs_revision'&&mine)action=`<button class="primary" data-revise-request="${r.id}">${r.proposed_data?.request_context==='project_delete'?'ارسال مجدد':'اصلاح و ارسال مجدد'}</button>`;
    ...
    else if(mine)action=`${r.proposed_data?.request_context==='project_delete'?'':`<button class="ghost" data-amend-request="${r.id}">ویرایش</button> `}<button class="ghost danger" data-cancel-request="${r.id}">لغو درخواست</button>`;
