...
window.openReview=id=>{
  ...
  $('#reviewDetails').innerHTML=`<p><b>نوع درخواست:</b> ${types[r.request_type]||r.request_type}</p><p><b>مرحله:</b> ${fa(r.current_stage||1)}</p><p><b>عنوان:</b> ${safe(r.proposed_data?.title||task?.title||'—')}</p><p><b>متولی:</b> ${safe(profileLabel(r.proposed_data?.owner_id,ownerName(task||{})))}</p><p><b>وضعیت پیشنهادی:</b> ${safe(r.proposed_data?.status||'—')}</p><p><b>توضیحات:</b> ${safe(r.proposed_data?.description??task?.description??'—')}</p><p><b>یادآور (روز):</b> ${fa(r.proposed_data?.reminder_days??task?.reminder_days??0)}</p><p><b>اولویت پیشنهادی:</b> ${safe(r.proposed_data?.priority||'—')}</p><p><b>تاریخ شروع:</b> ${jalaliText(r.proposed_data?.start_date)}</p><p><b>تاریخ پایان:</b> ${jalaliText(r.proposed_data?.due_date)}</p><p><b>تاریخ انجام:</b> ${jalaliText(r.proposed_data?.done_date)}</p>`;
