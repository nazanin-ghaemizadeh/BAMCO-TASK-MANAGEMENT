...
test('chat photos, search and own-message edit use real handlers without resending a message',async t=>{
 ...
 d.querySelector('[data-person="test-owner"]').click();await until(()=>d.querySelector('.messenger-compose'));const input=d.querySelector('.messenger-compose textarea');input.value='متن اولیه';d.querySelector('.messenger-compose').requestSubmit();await until(()=>d.querySelector('.message-edit'));assert(d.querySelector('.message-edit').classList.contains('bamco-icon-button'));assert.equal(d.querySelector('.message-edit').getAttribute('aria-label'),'ویرایش پیام');
...
test('current Persian month completion and create-request counts use the agreed denominator',async t=>{
 ...
 const values=[...d.querySelector('#performanceReportView tbody tr').cells].map(c=>c.textContent.trim());assert.equal(values[1],'۴');assert.equal(values[5],'۲');assert.equal(values[6],'۱');assert.equal(values[7],'۵۰٪');assert.equal(values[8],'۰');assert.equal(values[9],'۱');assert.equal(d.querySelector('#performanceReportView .workspace-metrics'),null);
});

test('performance report separates task-definition requests for self from requests for others',async t=>{
 const f=await fixture({tables:{tasks:[],change_requests:[],app_settings:[]}}),{w,d}=f;t.after(()=>f.dispose());const now=new Date(),month=new Intl.DateTimeFormat('fa-IR-u-nu-latn',{year:'numeric',month:'numeric',timeZone:'Asia/Tehran'}).formatToParts(now),y=Number(month.find(x=>x.type==='year').value),m=Number(month.find(x=>x.type==='month').value),from=w.jalaliToISO(y,m,1);f.tables.app_settings.push({key:'performance_monitoring_started_at',value:{value:from+'T08:30:00Z'}});
 f.tables.tasks.push({id:20,title:'مدیر',owner_id:'test-manager',created_by:'test-manager',created_at:from+'T08:00:00Z',status:'در حال انجام',archived:false,due_date:from},{id:21,title:'متولی',owner_id:'test-owner',created_by:'test-owner',created_at:from+'T09:00:00Z',status:'در حال انجام',archived:false,due_date:from});
 f.tables.change_requests.push({id:101,requested_by:'test-manager',request_type:'create',created_at:from+'T09:00:00Z',proposed_data:{owner_id:'test-owner'}},{id:102,requested_by:'test-manager',request_type:'create',created_at:from+'T10:00:00Z',proposed_data:{owner_id:'test-manager'}},{id:103,requested_by:'test-manager',request_type:'create',created_at:'2025-01-01T00:00:00Z',proposed_data:{owner_id:'test-owner'}});
 await w.eval('refresh()');await f.open('performanceReport');await until(()=>d.querySelector('[data-definition-metric="for-others"]'));const row=[...d.querySelectorAll('#performanceReportView tbody tr[data-workspace-index]')].find(item=>item.cells[0].textContent.includes('مدیر'));assert(row);assert.equal(row.querySelector('[data-definition-metric="for-others"]').textContent.trim(),'۱');assert.equal(row.querySelector('[data-definition-metric="for-self"]').textContent.trim(),'۱');assert.match(d.querySelector('#performanceReportView thead').textContent,/برای دیگران/);assert.match(d.querySelector('#performanceReportView thead').textContent,/برای خود/);assert.deepEqual(f.errors,[]);
});
