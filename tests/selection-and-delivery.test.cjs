...
test('manager performance definition metric counts only self-authored self-assigned tasks',async t=>{
 ...
 f.tables.tasks.push({id:20,title:'برای خود مدیر پیش از پایش',owner_id:'test-manager',created_by:'test-manager',created_at:from+'T08:00:00Z',status:'در حال انجام',archived:false,due_date:from},{id:21,title:'تعریف‌شده توسط متولی برای مدیر',owner_id:'test-manager',created_by:'test-owner',created_at:from+'T09:00:00Z',status:'در حال انجام',archived:false,due_date:from},{id:22,title:'تعریف مدیر برای متولی',owner_id:'test-owner',created_by:'test-manager',created_at:from+'T10:00:00Z',status:'در حال انجام',archived:false,due_date:from},{id:23,title:'برای خود مدیر پس از پایش',owner_id:'test-manager',created_by:'test-manager',created_at:from+'T11:00:00Z',status:'در حال انجام',archived:false,due_date:from});
 await w.eval('refresh()');await f.open('performanceReport');await until(()=>d.querySelector('[data-definition-metric="manager-self-task"]'));const metric=d.querySelector('[data-definition-metric="manager-self-task"]');assert.equal(metric.textContent.trim(),'۱');assert.match(metric.title,/شروع پایش/);assert.equal(d.querySelector('[data-definition-metric="owner-request"]').textContent.trim(),'۰');assert.deepEqual(f.errors,[]);
});
