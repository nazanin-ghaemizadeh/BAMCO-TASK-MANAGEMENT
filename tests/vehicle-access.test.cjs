...
test('manager saves scoped vehicle grants through the shared feature editor',async t=>{
 ...
 const dialog=f.d.querySelector('#vehicleAccessDialog');assert(dialog.classList.contains('permission-dialog'));assert(dialog.querySelector('[data-search]'));assert.equal(dialog.querySelector('.permission-actions'),null);dialog.querySelector('input[value="test-owner"]').click();dialog.querySelector('form').requestSubmit();await until(()=>f.calls.some(c=>c.endpoint==='set_feature_access'));const call=f.calls.find(c=>c.endpoint==='set_feature_access');assert.equal(call.body.p_feature_key,'vehicleTemporary');assert.deepEqual(call.body.p_grants,[{user_id:'test-owner',effect:'allow',can_view:true,can_create:true,can_edit:true,can_delete:true,can_export:true}]);assert.deepEqual(f.errors,[]);
});
