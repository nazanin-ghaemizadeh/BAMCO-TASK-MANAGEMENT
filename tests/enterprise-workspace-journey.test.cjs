...
test('enterprise pages keep a stable shared shell and persist the project, part and invoice journeys', async t => {
  ...
  assert.deepEqual([...projectForm.elements.status.options].map(option => option.textContent), ['ثبت شده', 'در حال انجام', 'منتظر پاسخ', 'انجام شده'], 'وضعیت پروژه دقیقاً از کاتالوگ کانبان می‌آید');
  ...
  assert.equal(tables.projects[0].status, 'ثبت شده');
  ...
  assert.deepEqual([...projectItemForm.elements.status.options].map(option => option.textContent), ['ثبت شده', 'در حال انجام', 'منتظر پاسخ', 'انجام شده'], 'فعالیت پروژه و کانبان یک فهرست وضعیت دارند');
