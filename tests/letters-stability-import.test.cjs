...
test('partial import retains failed rows and retry submits only the remaining rows',async t=>{
 ...
 w.bamcoOptions={load:async()=>{},normalizeTask:()=>{},status:()=>({archivable:true}),ordered:(_,v)=>v,label:(_,v)=>v};w.ensureBamcoXLSX=async()=>({read:()=>({Sheets:{S:{}},SheetNames:['S']}),utils:{sheet_to_json:()=>[{ID:1,title:'اول',owner:'فرد',description:0,manager_notes:0},{ID:2,title:'دوم',owner:'فرد'}]}});
 const calls=[];let fail=true;w.insert=async(_,body)=>{calls.push(body.legacy_id);if(body.legacy_id===1){assert.equal(body.description,'0');assert.equal(body.manager_notes,'0')}if(body.legacy_id===2&&fail)throw Error('خطای آزمایشی');return[{id:body.legacy_id}]};w.eval(fs.readFileSync('assets/js/tasks-io.js','utf8'));await w.BAMCO_DATA_IO.parse({arrayBuffer:async()=>new ArrayBuffer(0)});
