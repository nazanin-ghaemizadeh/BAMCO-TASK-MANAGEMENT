...
test('independent letters tab is available to explicitly granted owners; missing files cannot be downloaded and edits carry revision',async()=>{
 ...
 w.eval(fs.readFileSync('assets/js/letters.js','utf8'));w.document.dispatchEvent(new w.Event('DOMContentLoaded'));w.document.querySelector('#lettersView').classList.remove('hidden');await new Promise(r=>setTimeout(r,20));
