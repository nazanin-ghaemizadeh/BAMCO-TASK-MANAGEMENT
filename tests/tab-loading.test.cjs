...
for(const id of ['groupChat','directMessages','taskChats','loginActivity','activeSessions','performanceReport','responseReport','systemOptions'])test(`${id}: asynchronous data errors render a visible error panel`,async()=>{let shown;const fail=async()=>{throw Error('test unavailable')};const ctx={window:{bamcoTabs:{owns:id=>!['groupChat','directMessages','taskChats'].includes(id),render:fail},bamcoConversations:{owns:id=>['groupChat','directMessages','taskChats'].includes(id),render:fail}},REMOVED:new Set(['requestReport']),purgeRemoved:()=>{},panel:(...args)=>shown=args,esc:s=>s,can:()=>true,denied:()=>false,renderGroup:fail,renderDirect:fail,renderTasksChat:fail,renderSessions:fail,renderPerformance:fail,simpleReport:fail,systemOptions:fail,inflight:new Map(),Promise};vm.createContext(ctx);vm.runInContext(fn,ctx);await ctx.render(id);assert.equal(shown[0],id);assert.match(shown[3],/test unavailable/)});
