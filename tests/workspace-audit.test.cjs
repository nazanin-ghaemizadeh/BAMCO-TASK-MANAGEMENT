...
test('every navigation destination has one consistent toolbar return and releases the visible view',async t=>{
 ...
 for(const route of ['projects','parts','invoices','organization','tools']){await f.open(route);const view=d.querySelector('#'+route+'View');assert(view);assert.equal(view.querySelectorAll('[data-home-action]').length,1,route+' has one explicit return control');}
