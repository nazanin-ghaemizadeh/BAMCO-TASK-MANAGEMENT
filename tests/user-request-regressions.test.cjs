...
test('every nonzero workload segment, including one, receives a label',()=>{
  const chart=read('assets/js/reports-dashboard.js');
...
test('message center is one send flow and uses one renderer for portal and email',()=>{
  const center=read('assets/js/phase2-message-engine.js'),renderer=read('assets/js/message-renderer.js'),inbox=read('assets/js/message-history.js'),sql=read('supabase/migrations/20260911035403_unified_internal_messages_and_sent_log.sql');
...
test('task history is Persian and resolves owner names',()=>{
  const history=read('assets/js/message-history.js');
...
test('removed message-text editor cannot be reintroduced beside the canonical send flow',()=>{
  const rootFix=read('assets/js/messages-admin.js');
