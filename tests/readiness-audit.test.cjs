...
test('a background response report cannot cancel the visible performance report',async t=>{
 ...
 const f=await fixture({tables:{tasks:[{id:1,title:'QA',owner_id:'test-owner',status:'در حال انجام',archived:false,due_date:today}]}});t.after(()=>f.dispose());
 ...
 const pending=f.w.bamcoCanonicalReports.renderPerformance(true);await f.w.bamcoCanonicalReports.renderResponse(true);await pending;
...
test('ten selected recipients retain separate previews; queue failure can retry without duplicate preparation',async t=>{
 ...
 const cards=[...d.querySelectorAll('.snapshot-preview')];assert.equal(cards.length,10);cards.forEach((card,i)=>{assert(card.textContent.includes('خطاب اختصاصی '+i.toLocaleString('fa-IR')));for(let j=0;j<10;j++)if(j!==i)assert(!card.textContent.includes('خطاب اختصاصی '+j.toLocaleString('fa-IR')))});
