...
test('manager grants letters access through the generic feature editor without changing profiles',async t=>{
 ...
 await f.open('lettersIncoming');await until(()=>!f.d.querySelector('#lettersIncomingView [data-letter-action="access"]').classList.contains('hidden'));
 f.d.querySelector('#lettersIncomingView [data-letter-action="access"]').click();await until(()=>f.d.querySelector('#letterAccessDialog input[type=checkbox]'));
...
test('turning off effective letters access sends the explicit inherited-grant deny',async t=>{
 ...
 await f.open('lettersOutgoing');f.d.querySelector('#lettersOutgoingView [data-letter-action="access"]').click();await until(()=>f.d.querySelector('#letterAccessDialog [data-user-id="test-owner"]'));
