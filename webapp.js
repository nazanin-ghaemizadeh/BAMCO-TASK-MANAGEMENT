...
Promise.all(Array.from({length:n},(_,i)=>fetch(`webapp_parts/webapp_${String(i+1).padStart(2,'0')}.part?v=20260905-9`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`webapp part ${i+1}`);return r.text()}))).then(parts=>{
 ...
 Function(source)();
 const loadFinal=()=>{const f=document.createElement('script');f.src='final-fixes.js?v=20260905-9';document.head.appendChild(f)};
 const s=document.createElement('script');s.src='user-fixes.js?v=20260905-9';s.onload=loadFinal;s.onerror=loadFinal;document.head.appendChild(s);
