...
function renderTasks(archived){
  ...
  const body=archived?$('#archiveBody'):$('#kanbanBody');if(!rows.length){body.innerHTML=`<tr><td colspan="${archived?15:14}" class="empty">موردی برای نمایش وجود ندارد.</td></tr>`;return}
  body.innerHTML=rows.map(t=>{const due=norm(t.due_state),rowClass=due==='دیرکرد'?'row-overdue':due==='دوره هشدار'?'row-warning':'';
    const common=`<td>#${fa(displayId(t))}</td><td>${safe(t.title)}</td><td>${safe(t.description||'')}</td><td>${safe(ownerName(t))}</td><td>${safe(t.status)}</td><td>${safe(t.priority)}</td><td>${jalaliText(t.start_date)}</td><td>${jalaliText(t.done_date)}</td><td>${jalaliText(t.due_date)}</td><td>${fa(t.reminder_days)}</td><td>${jalaliDateTime(t.last_updated_at)}</td><td><i class="pill ${due==='دیرکرد'?'overdue':due==='دوره هشدار'?'warning':''}">${safe(t.due_state||'عادی')}</i></td><td>${safe(t.manager_notes||'')}</td>`;
    if(archived)return `<tr>${common}<td>${fa(t.delay_days||0)}</td><td>${fa(t.advance_days||0)}</td></tr>`;
    const editLabel=isManager()?'ویرایش':'درخواست تغییر',archiveLabel=isManager()?'تکمیل و آرشیو':'درخواست تکمیل';
    return `<tr class="${rowClass}">${common}<td><div class="row-actions"><button class="icon-btn" onclick="openEdit('${t.id}')">${editLabel}</button><button class="icon-btn" onclick="archiveTask('${t.id}')">${archiveLabel}</button></div></td></tr>`}).join('');
}
