...
const state={token:'',user:null,profile:null,profiles:[],tasks:[],requests:[],view:'dashboard',editing:null,reviewing:null};
...
async function enterApp(){
  ...
  $('#viewSubtitle').textContent=isManager()?'نمای کلی وظایف و عملکرد تیم':'فقط وظایف و عملکرد مربوط به شما';
  await refresh();showView('dashboard');
}
async function refresh(){
  try{state.profiles=isManager()?await select('profiles','select=id,email,full_name,role,active&active=eq.true&order=full_name'):[state.profile];
...
function renderTasks(archived){const query=(archived?$('#archiveSearch'):$('#kanbanSearch')).value.trim().toLowerCase();const rows=state.tasks.filter(t=>!!t.archived===archived).filter(t=>!query||[t.title,t.description,ownerName(t),t.status,t.priority,displayId(t)].some(v=>String(v??'').toLowerCase().includes(query)));const body=archived?$('#archiveBody'):$('#kanbanBody');
  if(!rows.length){body.innerHTML=`<tr><td colspan="${archived?7:8}" class="empty">موردی برای نمایش وجود ندارد.</td></tr>`;return}
  body.innerHTML=rows.map(t=>{const due=norm(t.due_state),rowClass=due==='دیرکرد'?'row-overdue':due==='دوره هشدار'?'row-warning':'';
    if(archived){let result='به‌موقع';if(t.done_date&&t.due_date)result=t.done_date>t.due_date?'تاخیر':t.done_date<t.due_date?'تعجیل':'به‌موقع';return `<tr><td>#${fa(displayId(t))}</td><td>${safe(t.title)}</td><td>${safe(ownerName(t))}</td><td><i class="pill ${t.priority==='فوری'?'urgent':''}">${safe(t.priority)}</i></td><td>${fa(t.due_date||'—')}</td><td>${fa(t.done_date||'—')}</td><td>${result}</td></tr>`}
    return `<tr class="${rowClass}"><td>#${fa(displayId(t))}</td><td>${safe(t.title)}</td><td>${safe(ownerName(t))}</td><td><i class="pill">${safe(t.status)}</i></td><td><i class="pill ${t.priority==='فوری'?'urgent':''}">${safe(t.priority)}</i></td><td>${fa(t.due_date||'—')}</td><td><i class="pill ${due==='دیرکرد'?'overdue':due==='دوره هشدار'?'warning':''}">${safe(t.due_state||'عادی')}</i></td><td><div class="row-actions"><button class="icon-btn" onclick="openEdit('${t.id}')">ویرایش</button><button class="icon-btn" onclick="archiveTask('${t.id}')">آرشیو</button></div></td></tr>`}).join('');
}
function renderRequests(){const names=Object.fromEntries(state.profiles.map(p=>[p.id,p.full_name||p.email]));const types={create:'افزودن',update:'ویرایش',archive:'آرشیو'};$('#approvalBadge').textContent=fa(state.requests.length);$('#approvalBadge').classList.toggle('hidden',!state.requests.length);
  $('#approvalBody').innerHTML=state.requests.length?state.requests.map(r=>`<tr><td>#${fa(r.id)}</td><td>${safe(names[r.requested_by]||'—')}</td><td><i class="pill">${types[r.request_type]||r.request_type}</i></td><td>${safe(r.proposed_data?.title||state.tasks.find(t=>String(t.id)===String(r.task_id))?.title||'—')}</td><td>${fa(new Date(r.created_at).toLocaleString('fa-IR'))}</td><td><button class="primary" onclick="openReview('${r.id}')">بررسی</button></td></tr>`).join(''):'<tr><td colspan="6" class="empty">درخواست منتظر تأییدی وجود ندارد.</td></tr>';
}
...
$('#kanbanSearch').addEventListener('input',()=>renderTasks(false));$('#archiveSearch').addEventListener('input',()=>renderTasks(true));
$('#collapseBtn').addEventListener('click',()=>$('#sidebar').classList.toggle('collapsed'));
$('#logoutBtn').addEventListener('click',()=>{sessionStorage.removeItem('bamco_session');location.reload()});
$$('[data-close]').forEach(b=>b.addEventListener('click',()=>document.getElementById(b.dataset.close).close()));

function fillOwners(selected){const sel=$('#taskForm [name=owner_id]');sel.innerHTML=state.profiles.map(p=>`<option value="${p.id}" ${p.id===selected?'selected':''}>${safe(p.full_name||p.email)}</option>`).join('')}
function openTask(task=null){state.editing=task;const f=$('#taskForm');f.reset();fillOwners(task?.owner_id||state.profile.id);$('#taskDialogTitle').textContent=task?'ویرایش تسک':'افزودن تسک';$('#taskDialogHint').textContent=isManager()?'تغییرات مدیر بلافاصله اعمال می‌شود.':'درخواست شما پس از تأیید مدیر اعمال می‌شود.';$('#saveTaskBtn').textContent=task?'ثبت تغییرات':'ثبت تسک';
  if(task)for(const key of ['title','description','status','priority','reminder_days','start_date','done_date','due_date','manager_notes'])if(f.elements[key])f.elements[key].value=task[key]??'';
  $('#taskDialog').showModal();
}
$('#addTaskBtn').addEventListener('click',()=>openTask());window.openEdit=id=>openTask(state.tasks.find(t=>String(t.id)===String(id)));
$('#taskForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;const data=Object.fromEntries(new FormData(f));for(const k of ['start_date','done_date','due_date'])if(!data[k])data[k]=null;data.reminder_days=Number(data.reminder_days||0);if(!isManager())data.owner_id=state.profile.id;
...
window.archiveTask=async id=>{const task=state.tasks.find(t=>String(t.id)===String(id));if(!confirm(`تسک «${task.title}» به آرشیو منتقل شود؟`))return;try{if(isManager())await update('tasks',`id=eq.${id}`,{archived:true,archived_at:new Date().toISOString(),status:'انجام شده',done_date:task.done_date||new Date().toISOString().slice(0,10)});else await insert('change_requests',{task_id:Number(id),request_type:'archive',proposed_data:{},requested_by:state.profile.id});toast(isManager()?'تسک آرشیو شد.':'درخواست آرشیو برای مدیر ارسال شد.');await refresh()}catch(err){toast(err.message,true)}};
window.openReview=id=>{state.reviewing=state.requests.find(r=>String(r.id)===String(id));const r=state.reviewing,types={create:'افزودن تسک',update:'ویرایش تسک',archive:'انتقال به آرشیو'};const task=state.tasks.find(t=>String(t.id)===String(r.task_id));$('#reviewDetails').innerHTML=`<p><b>نوع درخواست:</b> ${types[r.request_type]}</p><p><b>عنوان:</b> ${safe(r.proposed_data?.title||task?.title||'—')}</p><p><b>وضعیت پیشنهادی:</b> ${safe(r.proposed_data?.status||'—')}</p><p><b>اولویت پیشنهادی:</b> ${safe(r.proposed_data?.priority||'—')}</p><p><b>تاریخ پایان:</b> ${fa(r.proposed_data?.due_date||'—')}</p>`;$('#managerNote').value='';$('#reviewDialog').showModal()};
