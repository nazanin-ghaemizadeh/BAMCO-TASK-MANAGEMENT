...
function renderDashboard(){const active=state.tasks.filter(t=>!t.archived),archive=state.tasks.filter(t=>t.archived);
  ...
  const statuses=['ثبت شده','منتظر پاسخ','در حال انجام'];const max=Math.max(1,...statuses.map(s=>active.filter(t=>norm(t.status)===s).length));
...
function renderTasks(archived){
  ...
  const body=archived?$('#archiveBody'):$('#kanbanBody');if(!rows.length){body.innerHTML=`<tr><td colspan="${archived?16:14}" class="empty">موردی برای نمایش وجود ندارد.</td></tr>`;return}
  body.innerHTML=rows.map(t=>{const due=norm(t.due_state),rowClass=due==='دیرکرد'?'row-overdue':due==='دوره هشدار'?'row-warning':'';
    const common=`${cell(displayId(t),'#'+fa(displayId(t)))}${cell(t.title)}${cell(t.description||'')}${cell(ownerName(t))}${cell(t.status)}${cell(t.priority)}${cell(jalaliText(t.start_date))}${cell(jalaliText(t.done_date))}${cell(jalaliText(t.due_date))}${cell(fa(t.reminder_days))}${cell(jalaliDateTime(t.last_updated_at))}<td class="fa-text"><i class="pill ${due==='دیرکرد'?'overdue':due==='دوره هشدار'?'warning':''}">${safe(t.due_state||'عادی')}</i></td>${cell(t.manager_notes||'')}`;
    if(archived)return `<tr>${common}${cell(fa(t.delay_days||0))}${cell(fa(t.advance_days||0))}<td class="fa-text">${isManager()?`<button class="danger compact-danger" onclick="deleteTask('${t.id}')">حذف</button>`:'—'}</td></tr>`;
    const editLabel=isManager()?'ویرایش':'درخواست تغییر',archiveLabel=isManager()?'تکمیل و آرشیو':'درخواست تکمیل';
    return `<tr class="${rowClass}">${common}<td class="fa-text"><div class="row-actions"><button class="icon-btn" onclick="openEdit('${t.id}')">${editLabel}</button><button class="icon-btn" onclick="archiveTask('${t.id}')">${archiveLabel}</button>${isManager()?`<button class="danger compact-danger" onclick="deleteTask('${t.id}')">حذف</button>`:''}</div></td></tr>`}).join('');
}
function taskColumnValues(t,archived){const base=[displayId(t),t.title,t.description||'',ownerName(t),t.status,t.priority,jalaliText(t.start_date),jalaliText(t.done_date),jalaliText(t.due_date),fa(t.reminder_days),jalaliDateTime(t.last_updated_at),t.due_state||'عادی',t.manager_notes||''];return archived?[...base,fa(t.delay_days||0),fa(t.advance_days||0),'']: [...base,'']}
...
$('#passwordForm').addEventListener('submit',async e=>{e.preventDefault();const p=$('#newPassword').value,c=$('#confirmPassword').value;$('#passwordError').textContent='';if(p!==c){$('#passwordError').textContent='تکرار رمز عبور یکسان نیست.';return}const profileUpdate={must_change_password:false,updated_at:new Date().toISOString()};try{await update('profiles',`id=eq.${state.profile.id}`,profileUpdate);await api('/auth/v1/user',{method:'PUT',body:{password:p}});state.profile.must_change_password=false;$('#passwordDialog').close();toast('رمز عبور با موفقیت تغییر کرد.');showView('kanban')}catch(err){try{await update('profiles',`id=eq.${state.profile.id}`,{must_change_password:true,updated_at:new Date().toISOString()})}catch{}const message=err.message==='New password should be different from the old password.'?'رمز جدید باید با رمز قبلی متفاوت باشد.':err.message;$('#passwordError').textContent=message}});
