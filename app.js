...
function renderTasks(archived){
  ...
  body.innerHTML=rows.map(t=>{const due=norm(t.due_state),rowClass=due==='دیرکرد'?'row-overdue':due==='دوره هشدار'?'row-warning':'';
    ...
    if(archived)return `<tr class="${selected?'task-selected':''}" data-task-id="${t.id}" data-scope="archive">${common}${cell(fa(t.delay_days||0))}${cell(fa(t.advance_days||0))}${pick}</tr>`;
